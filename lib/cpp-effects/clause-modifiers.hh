...
template <typename Cmd> struct plain {};
...
public:
  ...
  invoke_command(std::list<metaframe_ptr>::iterator it,
...
template <typename Cmd> struct no_resume {};
...
public:
  [[noreturn]] virtual typename Cmd::out_type
  invoke_command(std::list<metaframe_ptr>::iterator it,
                 const Cmd &cmd) final override {
    // (continued from OneShot::InvokeCmd) ...looking for [d]
        ...
        .resume_with([&](ctx::fiber && /*prev*/) -> ctx::fiber {
          if constexpr (!std::is_void<Answer>::value) {
            *(static_cast<std::optional<Answer> *>(
                metastack.front()->return_buffer)) = this->handle_command(cmd);
          } else {
    ...
    fmt::println(stderr, "Malformed no_resume handler");
...
template <typename Cmd> struct no_manage {};
  ...
  invoke_command(std::list<metaframe_ptr>::iterator it,
    ...
    std::move(metastack.front()->fiber)
        .resume_with([&](ctx::fiber &&prev) -> ctx::fiber {
          ...
          // We don't need to keep the handler alive for the duration of the
          // command clause call (compare command_clause<Answer,
          // Cmd>::InvokeCmd)
