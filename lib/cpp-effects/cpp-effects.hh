...
template <typename... Outs> struct command;

template <typename Out> struct command<Out> {
  using out_type = Out;
...
template <typename Out, typename Answer> class resumption_data;

template <typename T> class resumption;

// Handlers

template <typename Answer, typename Body, typename... Cmds> class handler;

template <typename Answer, typename... Cmds> class flat_handler;
...
typename H::answer_type handle(int64_t label,
                               std::function<typename H::body_type()> body,
                               Args &&...args);

template <typename H, typename... Args>
typename H::answer_type handle(std::function<typename H::body_type()> body,
                               Args &&...args);

template <typename H>
typename H::answer_type handle_with(int64_t label,
                                    std::function<typename H::body_type()> body,
                                    std::shared_ptr<H> handler);

template <typename H>
typename H::answer_type handle_with(std::function<typename H::body_type()> body,
                                    std::shared_ptr<H> handler);

template <typename H, typename... Args>
typename H::answer_type
handle_ref(int64_t label,
           std::function<typename H::body_type(handler_ref)> body,
           Args &&...args);

template <typename H, typename... Args>
typename H::answer_type
handle_ref(std::function<typename H::body_type(handler_ref)> body,
           Args &&...args);

template <typename H>
typename H::answer_type
handle_with_ref(int64_t label,
                std::function<typename H::body_type(handler_ref)> body,
                std::shared_ptr<H> handler);

template <typename H>
typename H::answer_type
handle_with_ref(std::function<typename H::body_type(handler_ref)> body,
                std::shared_ptr<H> handler);

// Lifting a function to a resumption by wrapping it in a handler

template <typename H, typename... Args>
resumption<typename H::answer_type()>
wrap(int64_t label, std::function<typename H::body_type()> body,
     Args &&...args);

template <typename H, typename A, typename... Args>
resumption<typename H::answer_type(A)>
wrap(int64_t label, std::function<typename H::body_type(A)> body,
     Args &&...args);

template <typename H, typename... Args>
resumption<typename H::answer_type()>
wrap(std::function<typename H::body_type()> body, Args &&...args);

template <typename H, typename A, typename... Args>
resumption<typename H::answer_type(A)>
wrap(std::function<typename H::body_type(A)> body, Args &&...args);

template <typename H>
resumption<typename H::answer_type()>
wrap_with(int64_t label, std::function<typename H::body_type()> body,
          std::shared_ptr<H> handler);

template <typename H, typename A>
resumption<typename H::answer_type()>
wrap_with(int64_t label, std::function<typename H::body_type(A)> body,
          std::shared_ptr<H> handler);

template <typename H>
resumption<typename H::answer_type()>
wrap_with(std::function<typename H::body_type()> body,
          std::shared_ptr<H> handler);

template <typename H, typename A>
resumption<typename H::answer_type()>
wrap_with(std::function<typename H::body_type(A)> body,
          std::shared_ptr<H> handler);
...
template <typename Cmd> typename Cmd::out_type invoke_command(const Cmd &cmd);

template <typename Cmd>
typename Cmd::out_type invoke_command(handler_ref it, const Cmd &cmd);

template <typename H, typename Cmd>
typename Cmd::out_type static_invoke_command(int64_t goto_handler,
                                             const Cmd &cmd);
...
template <typename Cmd> handler_ref find_handler();
...
template <typename T> struct tangible {
  T value;
  tangible() = delete;
...
public:
  ...
  virtual void debug_print() const {
    fmt::println("{}:{}", label, typeid(*this).name());
  }
  metaframe() : label(0) {}
  int64_t label;
  ctx::fiber fiber;
...
template <typename Cmd> class can_invoke_command {
public:
  virtual typename Cmd::out_type
  invoke_command(std::list<cpp_effects_internals::metaframe_ptr>::iterator it,
                 const Cmd &cmd) = 0;
};
...
typename Cmd::out_type command_clause<Answer, Cmd>::invoke_command(
    std::list<cpp_effects_internals::metaframe_ptr>::iterator it,
    const Cmd &cmd) {
  ...
  rd.stored_metastack.splice(rd.stored_metastack.begin(), metastack,
                             metastack.begin(), it);
      ...
      .resume_with([&](ctx::fiber &&prev) -> ctx::fiber {
        ...
        if constexpr (!std::is_void<Answer>::value) {
          *(static_cast<std::optional<Answer> *>(
              ...
              this->handle_command(
                  cmd,
                  resumption<typename Cmd::template resumption_type<Answer>>(
                      rd));
        } else {
          this->handle_command(
              cmd,
              resumption<typename Cmd::template resumption_type<Answer>>(rd));
        }
        return ctx::fiber();
      });
...
class resumption_base {
  template <typename H>
  friend typename H::answer_type
  handle_with(int64_t label, std::function<typename H::body_type()> body,
              std::shared_ptr<H> handler);
...
class resumption_data final : public resumption_base {
  ...
  friend class cpp_effects_internals::command_clause;
  template <typename> friend class resumption;
...
template <typename Out, typename Answer> class resumption<Answer(Out)> {
public:
  resumption() {}
  ...
  ~resumption() {
    if (data) {
      ...
      std::list<cpp_effects_internals::metaframe_ptr> _(
          std::move(data->stored_metastack));
    }
  }
...
template <typename Answer> class resumption<Answer()> {
public:
  resumption() {}
  ...
  Answer resume() && { return release()->resume(); }
  Answer tail_resume() &&;
...
resumption<Answer(Out)>::resumption(std::function<Answer(Out)> func) {
  ...
  struct Arg : command<Out> {
    resumption<Answer(Out)> &res;
  };
  ...
  handle<HAbort>([&r, func]() {
    handle<HArg>([&r, func]() { return func(invoke_command(Arg{{}, r})); });
  });
...
Answer resumption_data<Out, Answer>::resume() {
  ...
  if constexpr (!std::is_void<Answer>::value) {
    ...
    std::move(this->stored_metastack.front()->fiber)
        .resume_with([&](ctx::fiber &&prev) -> ctx::fiber {
...
template <typename Answer> Answer resumption<Answer()>::tail_resume() && {
  // Trampoline back to handle
  cpp_effects_internals::tail_resumption = release();
  if constexpr (!std::is_void<Answer>::value) {
    return Answer();
...
class handler : public cpp_effects_internals::metaframe,
  ...
  template <typename H, typename Cmd>
  friend typename Cmd::out_type static_invoke_command(int64_t goto_handler,
                                                      const Cmd &cmd);
...
private:
  Answer run_return(cpp_effects_internals::tangible<void>) {
...
typename H::answer_type handle_with(int64_t label,
  ...
  ctx::fiber bodyFiber{/*std::alocator_arg, std::move(pf),*/
                       [&](ctx::fiber &&prev) -> ctx::fiber && {
                         ...
                         cpp_effects_internals::metaframe_ptr returnFrame(
                             std::move(metastack.front()));
                         metastack.pop_front();

                         std::move(metastack.front()->fiber)
                             .resume_with([&](ctx::fiber &&) -> ctx::fiber {
                               if constexpr (!std::is_void<Answer>::value) {
                                 *(static_cast<std::optional<Answer> *>(
                                     metastack.front()->return_buffer)) =
                                     std::static_pointer_cast<H>(returnFrame)
                                         ->run_return(std::move(b));
                               } else {
                                 std::static_pointer_cast<H>(returnFrame)
                                     ->run_return(std::move(b));
                               }
                         ...
                         fmt::system_error(-1, "error: impssible!\n");
                       }};
...
handle_ref(int64_t label,
           ...
           Args &&...args) {
  if constexpr (!std::is_void<typename H::answer_type>::value) {
    return handle_with_ref(label, body,
                           std::make_shared<H>(std::forward<Args>(args)...));
  } else {
    handle_with_ref(label, body,
                    std::make_shared<H>(std::forward<Args>(args)...));
  }
}
...
wrap_with(int64_t label, std::function<typename H::body_type(A)> body,
          std::shared_ptr<H> handler) {
  return resumption<typename H::answer_type()>(
      [=](A a) { return handle_with<H>(label, std::bind(body, a), handler); });
}
...
wrap_with(std::function<typename H::body_type(A)> body,
          std::shared_ptr<H> handler) {
  return resumption<typename H::answer_type()>(
      [=](A a) { return handle_with<H>(std::bind(body, a), handler); });
}
...
typename Cmd::out_type invoke_command(int64_t goto_handler, const Cmd &cmd) {
  ...
  if (auto canInvoke =
          std::dynamic_pointer_cast<can_invoke_command<Cmd>>(*it)) {
    return canInvoke->invoke_command(std::next(it), cmd);
  }
  fmt::println(stderr, "error: handler with id {} does not handle {}",
               goto_handler, typeid(Cmd).name());
...
typename Cmd::out_type invoke_command(handler_ref it, const Cmd &cmd) {
  ...
  fmt::print(stderr, "error: selected handler does not handle {}\n",
             typeid(Cmd).name());
  debug_print_metastack();
  exit(-1);
}
...
template <typename Cmd> handler_ref find_handler() {
  ...
  fmt::println(stderr,
               "error: cpp_effects::find_handler did not find a handler");
  debug_print_metastack();
  exit(-1);
}
