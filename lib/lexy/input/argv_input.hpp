...
class argv_sentinel
    : public _detail::sentinel_base<argv_sentinel, argv_iterator> {};

/// An iterator over the command-line arguments.
class argv_iterator
    : public _detail::bidirectional_iterator_base<argv_iterator, const char> {
public:
  constexpr argv_iterator() noexcept : _arg(nullptr), _c(nullptr) {}

  constexpr const char &deref() const noexcept { return *_c; }
...
#endif
  constexpr explicit argv_iterator(char **argument, char *c) noexcept
      : _arg(argument), _c(c) {}
...
namespace lexy {
template <typename Encoding = default_encoding> class argv_input {
public:
...
struct _argvsep : token_base<_argvsep> {
  template <typename Reader> struct tp {
    ...
    constexpr auto try_parse([[maybe_unused]] Reader reader) {
      ...
      if constexpr (std::is_same_v<Reader, lexy::input_reader<
    ...
    constexpr void report_error(Context &context, const Reader &reader) {
      auto err = lexy::error<Reader, lexy::expected_char_class>(
          reader.position(), "argv-separator");
...
namespace lexy {
template <>
inline constexpr auto token_kind_of<lexy::dsl::_argvsep> =
