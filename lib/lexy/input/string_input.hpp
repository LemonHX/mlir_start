...
template <typename Encoding = default_encoding> class string_input {
...
public:
  ...
  //=== access ===//
  constexpr const char_type *data() const noexcept { return _data; }
...
template <typename CharT>
string_input(const CharT *begin,
             const CharT *end) -> string_input<deduce_encoding<CharT>>;
template <typename CharT>
string_input(const CharT *data,
             std::size_t size) -> string_input<deduce_encoding<CharT>>;
template <typename View>
