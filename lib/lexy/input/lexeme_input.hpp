...
namespace lexy {
template <typename ParentInput> class lexeme_input {
  using _lexeme_type = lexeme_for<ParentInput>;
  using _input_ptr = std::conditional_t<input_is_view<ParentInput>, ParentInput,
                                        const ParentInput *>;
...
public:
  ...
  _lexeme_type lexeme() const { return _lexeme; }
