...
#include "lib/lexy/error.hpp"
#include "lib/lexy/input/base.hpp"
#include "lib/lexy/lexeme.hpp"
#include <cstring>
...
public:
  ...
  struct marker {
    ...
    constexpr iterator position() const noexcept { return _it; }
  };
...
public:
  ...
  //=== constructors ===//
  /// Allows the creation of an uninitialized buffer that is then filled by the
  /// user.
  class builder {
  public:
    ...
    char_type *data() const noexcept { return _buffer._data; }
    std::size_t size() const noexcept { return _buffer._size; }

    buffer finish() && noexcept { return LEXY_MOV(_buffer); }
  ...
  static buffer adopt(const char_type *data, std::size_t size,
    ...
    // We can cast away the const-ness, since we require that `data` came from a
    // buffer origionally, where it wasn't const.
  ...
  template <typename CharT,
            typename = _detail::require_secondary_char_type<encoding, CharT>>
  explicit buffer(
  ...
  explicit buffer(
      ...
      MemoryResource *resource = _detail::get_memory_resource<MemoryResource>())
      : buffer(reinterpret_cast<const char_type *>(begin),
               reinterpret_cast<const char_type *>(end), resource) {
    static_assert(sizeof(CharT) == sizeof(char_type));
  }
  ...
  buffer(const buffer &other)
      : buffer(other.data(), other.size(), other._resource.get()) {}
  ...
  ~buffer() noexcept {
    ...
    if constexpr (_has_sentinel)
      _resource->deallocate(
          _data, _detail::round_size_for_swar(_size + 1) * sizeof(char_type),
          alignof(char_type));
    else
      _resource->deallocate(_data, _size * sizeof(char_type),
                            alignof(char_type));
  }

  buffer &
  operator=(const buffer &other) // NOLINT: we do guard against self-assignment
  {
    // Create a temporary buffer that owns the same memory as other but with our
    // resource. We then move assign it to *this.
  ...
  buffer &operator=(buffer &&other) noexcept(std::is_empty_v<MemoryResource>) {
    if (*_resource == *other._resource) {
      ...
      // We do that by swapping - when other is destroyed it will free our
      // memory.
  ...
  //=== access ===//
  const char_type *data() const noexcept { return _data; }
...
private:
  char_type *allocate(std::size_t size) const {
    if constexpr (_has_sentinel) {
      ...
      auto memory = static_cast<char_type *>(_resource->allocate(
          mem_size * sizeof(char_type), alignof(char_type)));
...
buffer(const View &)
    -> buffer<deduce_encoding<LEXY_DECAY_DECLTYPE(*LEXY_DECLVAL(View).data())>>;
...
//=== make_buffer ===//
template <typename Encoding, encoding_endianness Endian> struct _make_buffer {
  template <typename MemoryResource = void>
  auto operator()(const void *_memory, std::size_t size,
                  MemoryResource *resource =
                      _detail::get_memory_resource<MemoryResource>()) const {
    constexpr auto native_endianness = LEXY_IS_LITTLE_ENDIAN
                                           ? encoding_endianness::little
                                           : encoding_endianness::big;
    ...
    if constexpr (sizeof(char_type) == 1 || Endian == native_endianness) {
      ...
      // The reinterpret_cast is technically UB, as we didn't create objects in
      // memory, but until std::start_lifetime_as is added, there is nothing we
      // can do.
      return buffer<Encoding, MemoryResource>(
          reinterpret_cast<const char_type *>(memory), size / sizeof(char_type),
          resource);
    } else {
      typename buffer<Encoding, MemoryResource>::builder builder(
          size / sizeof(char_type), resource);

      const auto end = memory + size;
      for (auto dest = builder.data(); memory != end;
           memory += sizeof(char_type)) {
        ...
        if constexpr (is_char16 && Endian == encoding_endianness::little)
          ...
          *dest++ =
          ...
          static_assert(_detail::error<Encoding>,
...
template <> struct _make_buffer<utf8_encoding, encoding_endianness::bom> {
  template <typename MemoryResource = void>
  auto operator()(const void *_memory, std::size_t size,
                  MemoryResource *resource =
                      _detail::get_memory_resource<MemoryResource>()) const {
    ...
    if (size >= 3 && memory[0] == 0xEF && memory[1] == 0xBB &&
        memory[2] == 0xBF) {
      memory += 3;
      size -= 3;
    }

    return _make_buffer<utf8_encoding, encoding_endianness::big>{}(memory, size,
                                                                   resource);
  }
};
template <> struct _make_buffer<utf8_char_encoding, encoding_endianness::bom> {
  template <typename MemoryResource = void>
  auto operator()(const void *_memory, std::size_t size,
                  MemoryResource *resource =
                      _detail::get_memory_resource<MemoryResource>()) const {
    ...
    return _make_buffer<utf8_char_encoding, encoding_endianness::big>{}(
        memory, size, resource);
  }
};
template <> struct _make_buffer<utf16_encoding, encoding_endianness::bom> {
  template <typename MemoryResource = void>
  auto operator()(const void *_memory, std::size_t size,
                  MemoryResource *resource =
                      _detail::get_memory_resource<MemoryResource>()) const {
    constexpr auto utf16_big =
        _make_buffer<utf16_encoding, encoding_endianness::big>{};
    constexpr auto utf16_little =
        _make_buffer<utf16_encoding, encoding_endianness::little>{};
    auto memory = static_cast<const unsigned char *>(_memory);
...
template <> struct _make_buffer<utf32_encoding, encoding_endianness::bom> {
  template <typename MemoryResource = void>
  auto operator()(const void *_memory, std::size_t size,
                  MemoryResource *resource =
                      _detail::get_memory_resource<MemoryResource>()) const {
    constexpr auto utf32_big =
        _make_buffer<utf32_encoding, encoding_endianness::big>{};
    constexpr auto utf32_little =
        _make_buffer<utf32_encoding, encoding_endianness::little>{};
    auto memory = static_cast<const unsigned char *>(_memory);

    if (size >= 4) {
      if (memory[0] == 0xFF && memory[1] == 0xFE && memory[2] == 0x00 &&
          memory[3] == 0x00)
        return utf32_little(memory + 4, size - 4, resource);
      else if (memory[0] == 0x00 && memory[1] == 0x00 && memory[2] == 0xFE &&
               memory[3])
...
constexpr auto make_buffer_from_input(
    const Input &input,
    MemoryResource *resource = _detail::get_memory_resource<MemoryResource>())
  ...
  if constexpr (_detail::is_detected<_detect_input_data, Input>) {
    ...
    if constexpr (std::is_pointer_v<decltype(begin)>) {
      ...
      for (auto cur = begin; cur != end; ++cur)
        *dest++ = *cur; // NOLINT: clang-analyzer thinks this might access zero
