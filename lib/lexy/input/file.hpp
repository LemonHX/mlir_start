...
using file_callback = void (*)(void *user_data, const char *memory,
                               std::size_t size);
...
public:
  ...
  explicit read_file_result(
      file_error ec, lexy::buffer<Encoding, MemoryResource> &&buffer) noexcept
...
struct _read_file_user_data {
  ...
  static auto callback() {
    return [](void *_user_data, const char *memory, std::size_t size) {
      ...
      user_data->buffer = lexy::make_buffer_from_raw<Encoding, Endian>(
          memory, size, user_data->resource);
    };
  }
};
