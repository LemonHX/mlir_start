...
public:
  ...
  constexpr explicit _rr(Iterator begin, Sentinel end) noexcept
      : _cur(begin), _end(end) {
  ...
  constexpr auto peek() const noexcept {
    if (_cur == _end)
      ...
      return encoding::to_int_type(
          static_cast<typename encoding::char_type>(*_cur));
  }
  ...
  constexpr iterator position() const noexcept { return _cur; }

  constexpr marker current() const noexcept { return {_cur}; }
...
LEXY_INSTANTIATION_NEWTYPE(_pr, _rr, Encoding,
...
namespace lexy {
template <typename Reader> struct _partial_input {
  Reader _reader;

  constexpr explicit _partial_input(Reader r) : _reader(r) {}

  constexpr Reader reader() const & { return _reader; }
};
...
template <typename Reader>
constexpr auto partial_input(const Reader &reader,
                             typename Reader::iterator end) {
