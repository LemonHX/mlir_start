...
template <typename Node> class parse_tree_encoding {
...
public:
  ...
  static LEXY_CONSTEVAL auto eof() { return _parse_tree_eof{}; }
...
public:
  ...
  constexpr _ptr child_reader() const & noexcept { return _ptr(_cur); }
  constexpr auto lexeme_reader() const & noexcept {
    auto lexeme = _traits::lexeme(_cur);
    return _range_reader<typename encoding::char_encoding>(lexeme.begin(),
                                                           lexeme.end());
  }

  constexpr const Node &peek() const noexcept { return _cur; }
  ...
  constexpr marker current() const noexcept { return {_parent, _cur}; }
  constexpr void reset(marker m) noexcept { _cur = m._cur; }

  constexpr iterator position() const noexcept { return current().position(); }
...
template <typename Node> class parse_tree_input {
public:
  using encoding = parse_tree_encoding<Node>;
  using value_type = Node;
  ...
  constexpr explicit parse_tree_input(Node root) noexcept
      : _root(LEXY_MOV(root)) {}
  ...
  //=== access ===//
  constexpr const Node &root() const noexcept { return _root; }

  //=== reader ===//
  constexpr auto reader() const & noexcept { return _ptr<Node>(_root); }
