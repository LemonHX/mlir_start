...
namespace lexy {
template <typename Reader, typename Tag> class error;

/// Type erased generic failure.
template <typename Reader> class error<Reader, void> {
public:
  ...
  constexpr const char *message() const noexcept { return _msg; }
  template <typename Tag> constexpr bool is(Tag = {}) const noexcept {
    return _detail::string_view(_msg) == _detail::type_name<Tag>();
  }

  constexpr auto begin() const noexcept { return _pos; }
...
template <typename Reader> class error<Reader, expected_literal> {
public:
  constexpr explicit error(typename Reader::iterator pos,
  ...
  constexpr auto string() const noexcept -> const
  ...
  constexpr auto character() const noexcept { return _str[_idx]; }
...
/// Unlike expected_literal, this one looks at the following characters as
/// well.
struct expected_keyword {};
template <typename Reader> class error<Reader, expected_keyword> {
public:
  constexpr explicit error(typename Reader::iterator begin,
                           typename Reader::iterator end,
                           const typename Reader::encoding::char_type *str,
  ...
  constexpr auto position() const noexcept { return _begin; }
...
template <typename Reader> class error<Reader, expected_char_class> {
public:
  constexpr explicit error(typename Reader::iterator pos,
...
/// Contains information about the context of an error, production is
/// type-erased.
template <typename Input> class error_context {
public:
  constexpr explicit error_context(
      lexy::production_info production, const Input &input,
  ...
  const char *production() const noexcept { return _production; }
