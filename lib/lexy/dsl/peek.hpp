...
struct peek_failure {
  static LEXY_CONSTEVAL auto name() { return "peek failure"; }
};

/// We've failed to match a peek not.
struct unexpected {
  static LEXY_CONSTEVAL auto name() { return "unexpected"; }
};
} // namespace lexy

namespace lexyd {
template <typename Rule, typename Tag> struct _peek : branch_base {
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      if (!impl.try_parse(context.control_block, reader)) {
        ...
        // But recover immediately, as we wouldn't have consumed anything either
        // way.
      }
  ...
  template <typename Error> static constexpr _peek<Rule, Error> error = {};
};

template <typename Rule, typename Tag> struct _peekn : branch_base {
  template <typename Reader> struct bp {
    typename Reader::iterator begin;
    typename Reader::marker end;
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      if (!impl.try_parse(context.control_block, reader)) {
        ...
        context.on(_ev::token{}, lexy::error_token_kind, impl.begin,
                   impl.end.position());
  ...
  template <typename Error> static constexpr _peekn<Rule, Error> error = {};
};

/// Check if at this reader position, the rule would match, but don't actually
/// consume any characters if it does.
template <typename Rule> constexpr auto peek(Rule) {
  return _peek<Rule, void>{};
}

/// Checks if at this reader position, the rule would not match.
template <typename Rule> constexpr auto peek_not(Rule) {
