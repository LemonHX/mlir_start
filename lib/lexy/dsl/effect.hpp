...
template <LEXY_NTTP_PARAM Fn> struct _eff : rule_base {
  static constexpr auto _fn() { return Fn; }
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      if constexpr (lexy::_detail::is_detected<_detect_eff_fn, _eff,
                                               state_type>) {
...
template <LEXY_NTTP_PARAM Fn> constexpr auto effect = _eff<Fn>{};
} // namespace lexyd
