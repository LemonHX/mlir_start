...
struct ascii_set {
  ...
  template <typename Fn> constexpr void visit(Fn fn) const {
    for (auto i = 0; i != 128; ++i)
      if (contains[i])
        fn(i);
  }

  template <typename Fn> constexpr void visit_range(Fn fn) const {
    auto range_begin = -1;
  ...
  constexpr void insert(int c) { contains[c] = true; }
...
struct compressed_ascii_set {
  ...
  static constexpr std::size_t range_count() { return RangeCount; }
  static constexpr std::size_t single_count() { return SingleCount; }
};

template <typename T> constexpr auto compress_ascii_set() {
...
template <const auto &CompressedAsciiSet, std::size_t... RangeIdx,
          std::size_t... SingleIdx>
struct ascii_set_matcher<CompressedAsciiSet, index_sequence<RangeIdx...>,
                         index_sequence<SingleIdx...>> {
  template <typename Encoding> static LEXY_CONSTEVAL auto to_int_type(char c) {
    return Encoding::to_int_type(static_cast<typename Encoding::char_type>(c));
  }

  template <typename Encoding>
  LEXY_FORCE_INLINE static constexpr bool
  match([[maybe_unused]] typename Encoding::int_type cur) {
    return
        // It must be in one of the ranges...
        ((to_int_type<Encoding>(CompressedAsciiSet.range_lower[RangeIdx]) <=
              cur &&
          cur <= to_int_type<Encoding>(
                     CompressedAsciiSet.range_upper[RangeIdx])) ||
         ...)
        // or one of the single characters.
        || ((cur ==
             to_int_type<Encoding>(CompressedAsciiSet.singles[SingleIdx])) ||
            ...);
  }
};
} // namespace lexy::_detail
...
struct char_class_base : token_base<Derived>, _char_class_base {
  ...
  static constexpr bool char_class_unicode() { return true; }

  static constexpr std::false_type char_class_match_cp(char32_t) { return {}; }

  template <typename Reader, typename Context>
  static constexpr void
  char_class_report_error(Context &context,
  ...
  template <typename Reader> struct tp {
    ...
    constexpr bool try_parse(Reader reader) {
      ...
      if constexpr (std::is_same_v<decltype(Derived::char_class_match_cp(
                                       char32_t())),
                                   std::false_type>) {
        return false;
      } else if constexpr (lexy::is_unicode_encoding<
    ...
    constexpr void report_error(Context &context, const Reader &reader) {
      Derived::template char_class_report_error<Reader>(context,
                                                        reader.position());
    }
  };
};
...
#define LEXY_CHAR_CLASS(Name, Rule)                                            \
  [] {                                                                         \
    ...
    struct c : ::lexyd::char_class_base<c> {                                   \
      ...
      static LEXY_CONSTEVAL auto char_class_name() { return Name; }            \
...
namespace lexyd {
template <typename CharT, CharT... C> struct _lit;
template <char32_t... Cp> struct _lcp;

// Implementation helper for the literal overloads.
template <char32_t Cp> struct _ccp : char_class_base<_ccp<Cp>> {
  static LEXY_CONSTEVAL auto char_class_name() { return "code-point"; }
...
template <unsigned char Byte> struct _cb : char_class_base<_cb<Byte>> {
  static constexpr auto char_class_unicode() { return Byte <= 0x7F; }

  static LEXY_CONSTEVAL auto char_class_name() { return "byte"; }
...
template <char32_t CP> constexpr auto _make_char_class(_lcp<CP>) {
  return _ccp<CP>{};
}
} // namespace lexyd

namespace lexyd {
template <typename... Cs> struct _calt : char_class_base<_calt<Cs...>> {
  static_assert(sizeof...(Cs) > 1);

  static constexpr auto char_class_unicode() {
    constexpr auto non_unicode = (!Cs::char_class_unicode() || ...);
    static_assert(!non_unicode
                      // If at least one is non-Unicode, either they all must be
                      // non-Unicode or only match ASCII.
  ...
  static LEXY_CONSTEVAL auto char_class_name() { return "union"; }
...
constexpr auto operator/(_calt<Cs...>,
                         C c) -> _calt<Cs..., decltype(_make_char_class(c))> {
  return {};
}
template <typename C, typename... Cs>
constexpr auto
operator/(C c, _calt<Cs...>) -> _calt<decltype(_make_char_class(c)), Cs...> {
  return {};
}
...
namespace lexyd {
template <typename C> struct _ccomp : char_class_base<_ccomp<C>> {
  static constexpr auto char_class_unicode() { return C::char_class_unicode(); }

  static LEXY_CONSTEVAL auto char_class_name() { return "complement"; }
  ...
  static constexpr auto char_class_match_cp(char32_t cp) {
    ...
    if constexpr (std::is_same_v<decltype(C::char_class_match_cp(cp)),
                                 std::false_type>)
      return true;
    else
...
template <typename C> constexpr auto operator-(_ccomp<C>) -> C { return {}; }
} // namespace lexyd
...
struct _cminus : char_class_base<_cminus<Set, Minus>> {
  ...
  static LEXY_CONSTEVAL auto char_class_name() { return "minus"; }
  ...
  static constexpr auto char_class_match_cp(char32_t cp) {
    if constexpr (std::is_same_v<decltype(Set::char_class_match_cp(cp)),
                                 std::false_type>)
      return std::false_type{};
    else if constexpr (std::is_same_v<decltype(Minus::char_class_match_cp(cp)),
                                      std::false_type>)
      // We don't match ASCII at this point: they only reach this point if the
      // ASCII table failed.
      return cp > 0x7F && Set::char_class_match_cp(cp);
    else
      // Same as above, no ASCII.
      return cp > 0x7F && Set::char_class_match_cp(cp) &&
             !Minus::char_class_match_cp(cp);
  }
};
...
namespace lexyd {
template <typename... Cs> struct _cand : char_class_base<_cand<Cs...>> {
  static_assert(sizeof...(Cs) > 1);
  ...
  static LEXY_CONSTEVAL auto char_class_name() { return "intersection"; }
...
template <typename... Cs, typename C>
constexpr auto operator&(_cand<Cs...>,
                         C c) -> _cand<Cs..., decltype(_make_char_class(c))> {
  return {};
}
template <typename C, typename... Cs>
constexpr auto
operator&(C c, _cand<Cs...>) -> _cand<decltype(_make_char_class(c)), Cs...> {
  return {};
}
