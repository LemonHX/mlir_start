...
#include "lib/lexy/code_point.hpp"
  ...
  static constexpr bool add_digit_checked(T &result, unsigned digit) {
    constexpr auto can_use_unsigned = [] {
      if constexpr (sizeof(T) >= sizeof(unsigned))
        // If it's bigger or of the same size as unsigned, we can't use
        // unsigned.
        return false;
      else {
        // We can do it if the worst-case does not overflow unsigned.
        auto worst_case = static_cast<unsigned>(_max);
        return integer_traits<unsigned>::add_digit_checked<Radix>(worst_case,
                                                                  Radix - 1);
      }
    }();
...
template <> struct integer_traits<code_point> {
  ...
  static constexpr void add_digit_unchecked(type &result, unsigned digit) {
    ...
    integer_traits<std::uint_least32_t>::add_digit_unchecked<Radix>(value,
                                                                    digit);
  ...
  static constexpr bool add_digit_checked(type &result, unsigned digit) {
    std::uint_least32_t value = result.value();
    if (!integer_traits<std::uint_least32_t>::add_digit_checked<Radix>(value,
                                                                       digit))
      return false;
    result = code_point(value);
...
template <typename T> struct integer_traits<unbounded<T>> {
...
template <typename T, T Max> struct bounded {};
template <typename T, T Max> struct integer_traits<bounded<T, Max>> {
  using type = typename integer_traits<T>::type;
  static constexpr auto is_bounded = true;
  ...
  template <int Radix>
  static constexpr bool add_digit_checked(type &result, unsigned digit) {
    return integer_traits<T>::template add_digit_checked<Radix>(result,
                                                                digit) &&
           result <= Max;
  }
};
...
struct integer_overflow {
  static LEXY_CONSTEVAL auto name() { return "integer overflow"; }
};
} // namespace lexy
...
constexpr bool _ndigits_can_overflow() {
  ...
  // so we have to be conservative and don't rule out overflow on the same
  // count.
  return N >= max_digit_count;
}

// Parses T in the Base without checking for overflow.
template <typename T, typename Base> struct _unbounded_integer_parser {
  using traits = lexy::integer_traits<T>;
  using base = Base;
...
struct _bounded_integer_parser {
  ...
  static constexpr auto radix = Base::digit_radix;
  static constexpr auto max_digit_count =
      traits::template max_digit_count<radix>;
  ...
  static constexpr result_type parse(Iterator cur, Iterator end) {
    ...
    for (std::size_t digit_count = 1; digit_count < max_digit_count;
         ++digit_count) {
      // Find the next digit.
      auto digit = 0u;
      do {
    ...
    // If we've reached this point, we've parsed the maximal number of digits
    // allowed. Now we can only overflow if there are still digits left.
    return {value, cur != end};
  }
};
template <typename T, typename Base, bool AssumeOnlyDigits>
using _integer_parser =
    std::conditional_t<_is_bounded<T>,
                       _bounded_integer_parser<T, Base, AssumeOnlyDigits>,
                       _unbounded_integer_parser<T, Base>>;

template <typename T, typename Digits> struct _integer_parser_digits;
...
struct _integer_parser_digits<T, _ndigits<N, Base>> {
  using value_type =
      std::conditional_t<_ndigits_can_overflow<T, N, Base::digit_radix>(), T,
                         lexy::unbounded<T>>;
...
struct _int : _copy_base<Token> {
  template <typename NextParser> struct _pc {
    template <typename Context, typename Reader, typename... Args>
    LEXY_PARSER_FUNC static bool
    parse(Context &context, Reader &reader, typename Reader::iterator begin,
      ...
      return lexy::whitespace_parser<Context, NextParser>::parse(
          context, reader, LEXY_FWD(args)..., value);
    }
  };
  ...
  template <typename Reader> struct bp {
    ...
    template <typename Context> constexpr void cancel(Context &) {}

    template <typename NextParser, typename Context, typename... Args>
    LEXY_PARSER_FUNC bool finish(Context &context, Reader &reader,
                                 Args &&...args) {
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
                                       Args &&...args) {
      auto begin = reader.position();
      if (lexy::token_parser_for<Token, Reader> parser(reader);
        ...
        if (begin == recovery_end) {
          ...
          context.on(_ev::token{}, lexy::digits_token_kind, begin,
                     recovery_end);
      ...
      return _pc<NextParser>::parse(context, reader, begin, end,
                                    LEXY_FWD(args)...);
    }
  };
};
...
template <typename T, typename Base>
struct _int_dsl
    : _int<_digits<lexy::_detail::type_or<Base, decimal>>,
           _integer_parser_for<T,
                               _digits<lexy::_detail::type_or<Base, decimal>>>,
           void> {
  template <typename Digits> constexpr auto operator()(Digits) const {
...
struct invalid_code_point {
  static LEXY_CONSTEVAL auto name() { return "invalid code point"; }
};
} // namespace lexy
...
constexpr auto code_point_id = [] {
  using type = std::conditional_t<
      _ndigits_can_overflow<lexy::code_point, N, Base::digit_radix>(),
...
struct invalid_code_unit {
  static LEXY_CONSTEVAL auto name() { return "invalid code unit"; }
};
} // namespace lexy
...
constexpr auto code_unit_id = [] {
  ...
  using type = std::conditional_t<
      _ndigits_can_overflow<char_type, N, Base::digit_radix>(), char_type,
      lexy::unbounded<char_type>>;
  using parser = _integer_parser<type, Base, true>;
