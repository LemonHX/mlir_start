...
struct _b : token_base<_b<N, Predicate>> {
  ...
  struct tp<Reader, lexy::_detail::index_sequence<Idx...>> {
    ...
    constexpr bool try_parse(Reader reader) {
      // Bump N times.
      auto result = ((_match(reader.peek()) ? (reader.bump(), true)
                                            : ((void)Idx, false)) &&
                     ...);
      end = reader.current();
      return result;
    ...
    constexpr void report_error(Context &context, const Reader &) {
      constexpr auto name = std::is_void_v<Predicate>
                                ? "byte"
                                : lexy::_detail::type_name<Predicate>();
      auto err =
          lexy::error<Reader, lexy::expected_char_class>(end.position(), name);
      context.on(_ev::error{}, err);
    }
  };

  //=== dsl ===//
  template <typename P> constexpr auto if_() const {
  ...
  constexpr auto range() const {
    struct predicate {
      static LEXY_CONSTEVAL auto name() { return "byte.range"; }
  ...
  template <unsigned char... Bytes> constexpr auto set() const {
    struct predicate {
      static LEXY_CONSTEVAL auto name() { return "byte.set"; }
  ...
  constexpr auto ascii() const {
    struct predicate {
      static LEXY_CONSTEVAL auto name() { return "byte.ASCII"; }
...
template <std::size_t N> constexpr auto bytes = _b<N, void>{};
} // namespace lexyd
...
//=== padding bytes ===//
namespace lexyd {
template <std::size_t N, unsigned char Padding = 0> struct _pb : branch_base {
  template <typename Context, typename Reader, typename Iterator>
  static constexpr void _validate(Context &context, const Reader &,
...
template <std::size_t N> auto _bint() {
  if constexpr (N == 1)
...
template <std::size_t N> using bint = decltype(_bint<N>());
} // namespace lexy::_detail

namespace lexy {
struct mismatched_byte_count {
  static LEXY_CONSTEVAL auto name() { return "mismatched byte count"; }
};
} // namespace lexy
...
struct _bint : branch_base {
  ...
  struct _pc<NextParser, lexy::_detail::index_sequence<Idx...>> {
    ...
    parse(Context &context, Reader &reader, typename Reader::iterator begin,
      ...
      return lexy::whitespace_parser<Context, NextParser>::parse(
          context, reader, LEXY_FWD(args)..., result);
    }
  };
