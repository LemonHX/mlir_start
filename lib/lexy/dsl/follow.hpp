...
struct follow_restriction {
  static LEXY_CONSTEVAL auto name() { return "follow restriction"; }
};
} // namespace lexy
...
struct _nf : token_base<_nf<Literal, CharClass>>, _lit_base {
  ...
  static constexpr auto lit_char_classes =
      lexy::_detail::char_class_list<CharClass>{};
  ...
  template <typename Reader> struct tp {
    ...
    constexpr void report_error(Context &context, Reader reader) {
      if (!literal_success) {
        ...
        auto err = lexy::error<Reader, lexy::follow_restriction>(
...
constexpr auto not_followed_by(Literal, CharClass cc) {
  static_assert(lexy::is_literal_rule<Literal> &&
                Literal::lit_char_classes.size == 0);
...
namespace lexy {
template <typename Literal, typename CharClass>
constexpr auto token_kind_of<lexy::dsl::_nf<Literal, CharClass>> =
