...
struct unexpected_trailing_separator {
  static LEXY_CONSTEVAL auto name() { return "unexpected trailing separator"; }
};
} // namespace lexy

namespace lexyd {
// Reports the trailing sep error.
template <typename Branch, typename Tag> struct _nsep : rule_base {
  template <typename NextParser> struct p {
    struct _pc {
      ...
      LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
        ...
        using tag =
            lexy::_detail::type_or<Tag, lexy::unexpected_trailing_separator>;
...
template <typename Branch, typename Tag> struct _sep : _sep_base {
  using rule = Branch;
  using trailing_rule = _nsep<Branch, Tag>;

  template <typename Context, typename Reader>
  static constexpr void
  report_trailing_error(Context &context, Reader &,
                        typename Reader::iterator sep_begin,
...
template <typename Branch> constexpr auto sep(Branch) {
  LEXY_REQUIRE_BRANCH_RULE(Branch, "sep");
  return _sep<Branch, void>{};
}

template <typename Branch> struct _tsep : _sep_base {
  using rule = Branch;
  using trailing_rule = decltype(lexyd::if_(Branch{}));

  template <typename Context, typename Reader>
  static constexpr void report_trailing_error(Context &, Reader &,
                                              typename Reader::iterator,
                                              typename Reader::iterator) {}
};

/// Defines a separator for a list that can be trailing.
template <typename Branch> constexpr auto trailing_sep(Branch) {
  LEXY_REQUIRE_BRANCH_RULE(Branch, "trailing_sep");
  return _tsep<Branch>{};
}

template <typename Branch> struct _isep : _sep_base {
  using rule = Branch;
...
/// Defines a separator for a list that ignores the existence of trailing
/// separators.
template <typename Branch> constexpr auto ignore_trailing_sep(Branch) {
