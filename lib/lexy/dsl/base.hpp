...
template <typename Rule> auto _copy_base_impl() {
...
template <typename Rule> using _copy_base = decltype(_copy_base_impl<Rule>());
} // namespace lexyd
...
struct _pb
    : lexy::branch_parser_for<lexy::production_rule<Production>, Reader> {};
// We create a new type here to shorten its name.
template <typename Production, typename Reader>
using production_branch_parser = _pb<Production, Reader>;

/// A branch parser that takes a branch unconditionally and forwards to the
/// regular parser.
template <typename Rule, typename Reader> struct unconditional_branch_parser {
  ...
  LEXY_PARSER_FUNC bool finish(Context &context, Reader &reader,
                               Args &&...args) {
    return parser_for<Rule, NextParser>::parse(context, reader,
                                               LEXY_FWD(args)...);
  }
};
...
struct continuation_branch_parser {
  ...
  LEXY_PARSER_FUNC bool finish(Context &context, Reader &reader,
                               Args &&...args) {
    return impl.template finish<Continuation<NextParser>>(context, reader,
                                                          LEXY_FWD(args)...);
  }
};

/// A parser that does not support any arguments.
template <typename... PrevArgs> struct pattern_parser {
  template <typename Context, typename Reader, typename... Args>
  LEXY_PARSER_FUNC static std::true_type
  parse(Context &, Reader &, const PrevArgs &..., Args &&...) {
    // A rule is used inside a loop or similar situation, where it must not
    // produce values, but it did.
...
struct sink_parser {
  ...
  LEXY_PARSER_FUNC static std::true_type parse(Context &, Reader &, Sink &sink,
                                               Args &&...args) {
...
template <typename NextParser> struct sink_finish_parser {
  template <typename Context, typename Reader, typename Sink, typename... Args>
  LEXY_PARSER_FUNC static auto parse(Context &context, Reader &reader,
                                     Sink &sink, Args &&...args) {
...
namespace lexy {
template <
    typename Context, typename NextParser,
    typename = lexy::production_whitespace<
...
// This is both an optimization and also doesn't require inclusion of
// whitespace.hpp.
...
LEXY_FORCE_INLINE constexpr auto try_match_token(TokenRule, Reader &reader) {
  ...
  if constexpr (std::is_same_v<try_parse_result, std::true_type>) {
    ...
    // try_parse() is pure and we don't want to advance the reader, so no need
    // to call it.
