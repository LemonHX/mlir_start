...
namespace lexyd {
template <typename Token> struct _cap : _copy_base<Token> {
...
template <typename Rule> struct _capr : _copy_base<Rule> {
  ...
  struct _pc : lexy::_detail::disable_whitespace_skipping {
    ...
    parse(Context &context, Reader &reader, PrevArgs &&...prev_args,
  ...
  template <typename Reader> struct bp {
    ...
    LEXY_PARSER_FUNC auto finish(Context &context, Reader &reader,
      ...
      return rule.template finish<continuation>(
          context, reader, LEXY_FWD(args)..., reader.position());
    }
  };
...
/// Captures whatever the token matches as a lexeme; does not include trailing
/// whitespace.
template <typename Token> constexpr auto capture(Token) {
  static_assert(lexy::is_token_rule<Token>);
  return _cap<Token>{};
}

/// Captures whatever the token production matches as lexeme; does not include
/// trailing whitespace.
template <typename Production> constexpr auto capture(_prd<Production>) {
