...
struct different_identifier {
  static LEXY_CONSTEVAL auto name() { return "different identifier"; }
};
} // namespace lexy
...
template <typename Id> struct _ctx_icreate : rule_base {
...
template <typename Id, typename Identifier> struct _ctx_icap : branch_base {
  ...
  template <typename NextParser> struct p {
    template <typename... PrevArgs> struct _cont {
      ...
      LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
        ...
        if (!lexy::_detail::equal_lexemes(
                _ctx_id<Id, Reader>::get(context.control_block), lexeme)) {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      return lexy::parser_for<_cap<_pattern>, _cont<Args...>>::parse(
          context, reader, LEXY_FWD(args)...);
    }
  };
...
namespace lexyd {
template <typename Id, typename Identifier> struct _ctx_id_dsl {
  constexpr auto create() const { return _ctx_icreate<Id>{}; }

  constexpr auto capture() const { return _ctx_icap<Id, Identifier>{}; }

  constexpr auto rematch() const { return _ctx_irem<Id, Identifier, void>{}; }
};

/// Declares a context variable that stores one instance of the given
/// identifier.
template <typename Id, typename Leading, typename Trailing,
          typename... Reserved>
