...
namespace lexy {
template <typename Production, typename Handler, typename State,
          typename Reader>
struct _subgrammar;

template <typename Production, typename Action>
using _subgrammar_for =
    _subgrammar<Production, typename Action::handler, typename Action::state,
...
#define LEXY_DECLARE_SUBGRAMMAR(Production)                                    \
  ...
  struct _subgrammar<Production, Handler, State, Reader> {                     \
    template <typename T>                                                      \
    static bool                                                                \
    parse(_detail::lazy_init<T> &value,                                        \
...
#define LEXY_DEFINE_SUBGRAMMAR(Production)                                     \
  template <typename Handler, typename State, typename Reader>                 \
  template <typename T>                                                        \
  bool ::lexy::_subgrammar<Production, Handler, State, Reader>::parse(         \
...
#define LEXY_INSTANTIATE_SUBGRAMMAR(Production, ...)                           \
  template bool ::lexy::_subgrammar_for<Production, __VA_ARGS__> /**/          \
      ...
      (::lexy::_detail::lazy_init<::lexy::_production_value_type<              \
           typename __VA_ARGS__::handler, typename __VA_ARGS__::state,         \
           Production>> &,                                                     \
...
namespace lexyd {
template <typename Production, typename T> struct _subg : rule_base {
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      constexpr auto production_uses_void_callback =
          std::is_same_v<typename handler_type::template value_callback<
                             Production, state_type>,
                         lexy::_detail::void_value_callback>;
      using value_type =
          std::conditional_t<production_uses_void_callback, void, T>;
      lexy::_detail::lazy_init<value_type> value;

      using subgrammar_traits =
          lexy::_subgrammar<Production, handler_type, state_type, Reader>;
      auto rule_result = subgrammar_traits::template parse<value_type>(
          value, context.control_block, reader);
...
/// Parses the entry production of a subgrammar, which may be defined in a
/// different file.
