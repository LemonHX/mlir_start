...
struct missing_delimiter {
  static LEXY_CONSTEVAL auto name() { return "missing delimiter"; }
};
} // namespace lexy

namespace lexyd {
template <typename CharClass, typename Reader> struct _del_chars {
  typename Reader::iterator begin;

  constexpr _del_chars(const Reader &reader) : begin(reader.position()) {}

  template <typename Context>
  constexpr void _recover(Context &context,
    ...
    context.on(_ev::token{}, lexy::error_token_kind, recover_begin,
               recover_end);
  ...
  constexpr void parse_swar(Reader &reader, Close, Escs...) {
    ...
    // If we have a SWAR reader and the Close and Escape chars are literal
    // rules, we can munch as much content as possible in a fast loop. We also
    // need to efficiently check for the CharClass for it to make sense.
    if constexpr (lexy::_detail::is_swar_reader<Reader> //
                  && (lexy::is_literal_rule<Close> && ... &&
                      Escs::esc_is_literal) &&
                  !std::is_same_v<
                      decltype(CharClass::template char_class_match_swar<
                               encoding>({})),
                      std::false_type>) {
      ...
      while (true) {
        ...
        // If we have an EOF or the initial character of the closing delimiter,
        // we exit as we have no more content.
        if (swar_has_char<char_type, encoding::eof()>(cur) ||
            swar_has_char<char_type,
                          Close::template lit_first_char<encoding>()>(cur))
          break;

        // The same is true if we have the escape character.
        if constexpr (sizeof...(Escs) > 0) {
          if ((swar_has_char<char_type,
                             Escs::template esc_first_char<encoding>()>(cur) ||
               ...))
            break;
        }

        // We definitely don't have the end of the delimited content in the
        // current SWAR, check if they all follow the char class.
  ...
  // Precondition: the next code unit definitely belongs to the content, not the
  // delimiter.
  ...
  constexpr void parse_one(Context &context, Reader &reader, Sink &sink) {
    ...
    if (matcher::template match<encoding>(reader.peek())) {
      reader.bump();
                             ...
                             decltype(CharClass::char_class_match_cp(
                                 char32_t())),
                             std::false_type>) {
  ...
  template <typename Context, typename Sink>
  constexpr void finish(Context &context, Sink &sink,
...
struct _del_limit {
  ...
  template <typename Reader> static constexpr bool peek(Reader reader) {
    return lexy::try_match_token(Token{}, reader) ||
           reader.peek() == Reader::encoding::eof();
  }
};
template <typename Error> struct _del_limit<void, Error> {
  using error = Error;
...
struct _del : rule_base {
  using _limit =
      std::conditional_t<std::is_void_v<Limit> || lexy::is_token_rule<Limit>,
                         _del_limit<Limit>, Limit>;

  template <typename CloseParser, typename Context, typename Reader,
            typename Sink>
  LEXY_PARSER_FUNC static bool _loop(CloseParser &close, Context &context,
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      if constexpr (std::is_same_v<typename decltype(sink)::return_type,
                                   void>) {
        LEXY_MOV(sink).finish();
        return close.template finish<NextParser>(context, reader,
                                                 LEXY_FWD(args)...);
      } else {
        return close.template finish<NextParser>(
            context, reader, LEXY_FWD(args)..., LEXY_MOV(sink).finish());
      }
    }
  };
};
...
struct _delim_dsl {
  ...
  constexpr auto open() const { return Open{}; }
...
template <typename Open, typename Close> constexpr auto delimited(Open, Close) {
...
template <typename Delim> constexpr auto delimited(Delim) {
...
struct invalid_escape_sequence {
  static LEXY_CONSTEVAL auto name() { return "invalid escape sequence"; }
};
} // namespace lexy

namespace lexyd {
template <typename Escape, typename... Branches> struct _escape : _escape_base {
  ...
  static constexpr bool esc_try_parse(Context &context, Reader &reader,
                                      Sink &sink,
    ...
    auto try_parse_branch = [&](auto branch) {
      ...
      // it will definitely consume the escape token, and everything that is a
      // valid prefix. The remaining stuff is then just treated as part of the
      // delimited.
    ...
    if constexpr ((lexy::is_unconditional_branch_rule<Branches> || ...)) {
      ...
      auto err = lexy::error<Reader, lexy::invalid_escape_sequence>(
  ...
  template <typename Branch> constexpr auto rule(Branch) const {
    LEXY_REQUIRE_BRANCH_RULE(Branch, "escape()");
    return _escape<Escape, Branches..., Branch>{};
  }

  /// Adds an escape rule that captures the branch.
  template <typename Branch> constexpr auto capture(Branch branch) const {
    LEXY_REQUIRE_BRANCH_RULE(Branch, "escape()");
  ...
  template <const auto &Table> constexpr auto symbol() const {
...
template <typename EscapeToken> constexpr auto escape(EscapeToken) {
