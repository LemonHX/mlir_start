...
#include "lib/lexy/callback/forward.hpp"
...
namespace lexyd {
template <typename Production> struct _prd;
template <typename Rule, typename Tag> struct _peek;
template <typename Token> struct _capt;
template <typename T, typename Base> struct _int_dsl;

struct _scan;
} // namespace lexyd

namespace lexy::_detail {
template <typename Derived, typename Reader> class scanner;
}
...
template <typename T> class scan_result {
public:
  using value_type = T;
  ...
  constexpr decltype(auto) value() const & noexcept { return *_value; }
  constexpr decltype(auto) value() && noexcept { return LEXY_MOV(*_value); }
...
private:
  constexpr explicit scan_result(_detail::lazy_init<T> &&value)
      : _value(LEXY_MOV(value)) {}

  _detail::lazy_init<T> _value;

  template <typename Derived, typename Reader> friend class _detail::scanner;
};
template <> class scan_result<void> {
public:
...
private:
  constexpr explicit scan_result(_detail::lazy_init<void> &&value)
      : _value(LEXY_MOV(value)) {}
...
template <typename T> scan_result(T &&) -> scan_result<std::decay_t<T>>;
template <typename T> scan_result(_detail::lazy_init<T> &&) -> scan_result<T>;
} // namespace lexy

//=== scanner implementation ===//
namespace lexy::_detail {
template <typename Reader> struct scanner_input {
  Reader _impl;

  constexpr auto reader() const & { return _impl; }
};

struct scan_final_parser {
  template <typename Context, typename Reader, typename T>
  LEXY_PARSER_FUNC static bool parse(Context &, Reader &, lazy_init<T> *dest,
                                     T &&value) {
  ...
  LEXY_PARSER_FUNC static bool parse(Context &, Reader &,
                                     lazy_init<void> *dest) {
    dest->emplace();
    return true;
  }
...
public:
  ...
  constexpr void parse(scan_result<T> &result, Rule) {
    ...
    auto success = parser::parse(static_cast<Derived &>(*this).context(),
                                 _reader, &result._value);
    if (!success)
  ...
  constexpr auto parse(Production = {}) {
    using context_t =
        LEXY_DECAY_DECLTYPE(static_cast<Derived &>(*this).context());
    using value_type = typename lexy::production_value_callback<
        Production, typename context_t::state_type>::return_type;
  ...
  constexpr bool branch(scan_result<T> &result, Rule) {
    ...
    auto success = parser.template finish<lexy::_detail::scan_final_parser>(
        context, _reader, &result._value);
    if (!success)
      _state = _state_failed;
  ...
  private:
    constexpr explicit error_recovery_guard(scanner &self) noexcept
        : _self(&self) {
      auto &context = static_cast<Derived &>(*_self).context();
  ...
  template <typename TokenRule> constexpr bool discard(TokenRule rule) {
  ...
  constexpr void error(Tag, Args &&...args) {
    auto &context = static_cast<Derived &>(*this).context();
    context.on(parse_events::error{},
               lexy::error<Reader, Tag>(LEXY_FWD(args)...));
  }
  ...
  constexpr void error(const char *msg, Args &&...args) {
    auto &context = static_cast<Derived &>(*this).context();
    context.on(parse_events::error{},
               lexy::error<Reader, void>(LEXY_FWD(args)..., msg));
  }
  ...
  template <typename Rule> constexpr bool peek(Rule) {
  ...
  template <typename T, typename Digits> constexpr auto integer(Digits digits) {
    scan_result<T> result;
  ...
  template <typename Token> constexpr auto capture_token(Token) {
...
protected:
  ...
  constexpr Reader &reader() noexcept { return _reader; }
...
template <typename Context, typename Reader>
class rule_scanner
    : public _detail::scanner<rule_scanner<Context, Reader>, Reader> {
public:
...
private:
  constexpr explicit rule_scanner(Context &context, Reader reader)
      : _detail::scanner<rule_scanner<Context, Reader>, Reader>(reader),
        _context(&context), _begin(reader.position()) {}
...
using _detect_scan_state =
    decltype(Context::production::scan(LEXY_DECLVAL(Scanner &), *StatePtr()));

struct _scan : rule_base {
  template <typename NextParser> struct p {
    template <typename Scanner, typename Context, typename Reader,
              typename... Args>
    LEXY_PARSER_FUNC static bool _parse(Scanner &scanner, Context &context,
      ...
      typename Context::production::scan_result result = [&] {
        if constexpr (lexy::_detail::is_detected<
          ...
          return Context::production::scan(
              scanner, *context.control_block->parse_state, LEXY_FWD(args)...);
        else
