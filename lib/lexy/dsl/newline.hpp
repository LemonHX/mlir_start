...
struct expected_newline {
  static LEXY_CONSTEVAL auto name() { return "expected newline"; }
};
} // namespace lexy

namespace lexyd {
struct _nl : LEXY_DECAY_DECLTYPE(literal_set(LEXY_LIT("\n"), LEXY_LIT("\r\n"))
...
struct _eol : branch_base {
  template <typename Reader> struct bp {
    ...
    LEXY_PARSER_FUNC bool finish(Context &context, Reader &reader,
      ...
      if (reader.peek() == Reader::encoding::eof()) {
        ...
        return lexy::parser_for<_nl, NextParser>::parse(context, reader,
                                                        LEXY_FWD(args)...);
      }
    }
  };
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
                                       Args &&...args) {
      static_assert(lexy::is_char_encoding<typename Reader::encoding>);
      return bp<Reader>{}.template finish<NextParser>(context, reader,
                                                      LEXY_FWD(args)...);
    }
  };
};
