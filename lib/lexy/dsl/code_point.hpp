...
namespace lexyd {
template <typename Predicate> struct _cp : char_class_base<_cp<Predicate>> {
  ...
  template <char32_t Low, char32_t High> constexpr auto range() const {
    struct predicate {
      static LEXY_CONSTEVAL auto name() { return "code-point.range"; }
  ...
  template <char32_t... CPs> constexpr auto set() const {
    struct predicate {
      static LEXY_CONSTEVAL auto name() { return "code-point.set"; }
  ...
  constexpr auto ascii() const {
    struct predicate {
      static LEXY_CONSTEVAL auto name() { return "code-point.ASCII"; }
  ...
  constexpr auto bmp() const {
    struct predicate {
      static LEXY_CONSTEVAL auto name() { return "code-point.BMP"; }
  ...
  constexpr auto noncharacter() const {
    struct predicate {
      static LEXY_CONSTEVAL auto name() { return "code-point.non-character"; }
  ...
  constexpr auto general_category() const {
    struct predicate {
      ...
      constexpr bool operator()(lexy::code_point cp) const {
        // Note: can't use `cp.is_noncharacter()` for `Cn` as `Cn` also
        // includes all code points that are currently unassigned.
  ...
  template <const auto &GcGroup> struct _group_pred;
  template <lexy::code_point::general_category_t... Cats,
            const lexy::code_point::_gc_group<Cats...> &GcGroup>
  struct _group_pred<GcGroup> {
    static LEXY_CONSTEVAL auto name() { return GcGroup.name; }

    constexpr bool operator()(lexy::code_point cp) const {
      return cp.general_category() == GcGroup;
    }
  };
  template <const auto &GcGroup> constexpr auto general_category() const {
...
namespace lexy {
// The void-version without predicate logically matches any input (modulo
// encoding errors, of course).
template <>
inline constexpr auto token_kind_of<lexy::dsl::_cp<void>> =
    lexy::any_token_kind;
} // namespace lexy
