...
struct duplicate_flag {
  static LEXY_CONSTEVAL auto name() { return "duplicate flag"; }
};
} // namespace lexy

namespace lexyd {
template <const auto &Table, typename Token, typename Tag> struct _sym;
...
struct _flags : rule_base {
  ...
  template <typename NextParser> struct p {
    ...
    static constexpr bool _parse(_int_type &result, Context &context,
                                 Reader &reader) {
      ...
      while (true) {
        ...
        if (!bp.template finish<lexy::pattern_parser<_enum_type>>(context,
                                                                  reader))
          return false;
        ...
        if ((result & flag) == flag) {
          using tag =
              lexy::_detail::type_or<DuplicateError, lexy::duplicate_flag>;
...
namespace lexyd {
template <typename Rule, auto If, auto Else> struct _flag : rule_base {
  template <typename NextParser> struct p {
    template <typename Context, typename Reader, typename... Args>
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
                                       Args &&...args) {
...
template <typename Rule> constexpr auto flag(Rule rule) {
