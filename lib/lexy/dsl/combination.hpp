...
struct combination_duplicate {
  static LEXY_CONSTEVAL auto name() { return "combination duplicate"; }
};
} // namespace lexy

namespace lexyd {
template <typename Sink> struct _comb_control {
...
struct _comb : rule_base {
  ...
  using _comb_choice =
      decltype(_comb_choice_(lexy::_detail::index_sequence_for<R...>{}));
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      for (auto count = 0; count < int(N); ++count) {
        ...
        using parser =
            lexy::parser_for<_comb_choice,
                             lexy::pattern_parser<decltype(control)>>;
        ...
        if (handled[control.idx]) {
          using tag = lexy::_detail::type_or<DuplicateError,
...
template <typename... R> constexpr auto combination(R...) {
...
template <typename... R> constexpr auto partial_combination(R...) {
