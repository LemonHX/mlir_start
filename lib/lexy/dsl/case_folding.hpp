...
//=== generic rule impl ===//
namespace lexyd {
template <template <typename> typename CaseFolding> struct _cfl_folding {
...
namespace lexy {
template <typename Literal, template <typename> typename CaseFolding>
constexpr auto token_kind_of<lexy::dsl::_cfl<Literal, CaseFolding>> =
...
struct _acfr // ascii case folding reader
{
  ...
  constexpr void bump() { _impl.bump(); }

  constexpr iterator position() const { return _impl.position(); }

  constexpr marker current() const noexcept { return _impl.current(); }
  constexpr void reset(marker m) noexcept { _impl.reset(m); }
};
} // namespace lexy

namespace lexyd::ascii {
struct _cf_dsl {
  template <typename Encoding> static constexpr auto is_inplace = true;

  template <typename Reader> using case_folding = lexy::_acfr<Reader>;

  template <typename Literal> constexpr auto operator()(Literal) const {
    static_assert(lexy::is_literal_rule<Literal>);
    static_assert(std::is_void_v<typename Literal::lit_case_folding>,
...
struct _sucfrm // simple unicode case folding reader, UTF-8 and UTF-16
{
  ...
  constexpr explicit _sucfrm(Reader impl)
      : _impl(impl), _cur_pos(_impl.current()), _buffer{}, _buffer_size(0),
        _buffer_cur(0) {
    _fill();
  }
  ...
  constexpr void _fill() {
    ...
    if (result.error == lexy::_detail::cp_error::success) {
      ...
      _buffer_size =
          static_cast<unsigned char>(lexy::_detail::encode_code_point<encoding>(
              folded.value(), _buffer, 4));
      _buffer_cur = 0;
      ...
      while (_impl.position() != result.end.position()) {
        _buffer[_buffer_size] =
            static_cast<typename encoding::char_type>(_impl.peek());
        ++_buffer_size;
        _impl.bump();
      }
    }
  ...
  constexpr iterator position() const { return current().position(); }

  constexpr marker current() const noexcept {
    // We only report a marker at a code point boundary.
    // This has two consequences:
    // 1. If we don't match a rule, the error token does not include any common
    // start code
    //    units. That's actually nice, and makes it unnecessary to handle that
    //    situation in the error reporting. The only relevant difference is in
    //    the error token.
    // 2. If the user wants to match partial code unit sequences, the behavior
    // can become buggy.
...
template <typename Reader>
using _sucfr_for = std::conditional_t<
    std::is_same_v<typename Reader::encoding, lexy::utf32_encoding>,
    _sucfr32<Reader>, _sucfrm<Reader>>;

template <typename Reader> struct _sucfr : _sucfr_for<Reader> {
...
struct _scf_dsl {
  ...
  template <typename Reader> using case_folding = lexy::_sucfr<Reader>;
...
/// Matches Literal with case insensitive Unicode characters (simple case
/// folding).
