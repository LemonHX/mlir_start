...
constexpr void _until_swar([[maybe_unused]] Reader &reader) {
  if constexpr (std::is_same_v<Condition, _nl> //
    ...
    while (true) {
      ...
      if (lexy::_detail::swar_has_char<char_type, Reader::encoding::eof()>(
              cur) ||
...
struct _until_eof
    : token_base<_until_eof<Condition>, unconditional_branch_base> {
  template <typename Reader> struct tp {
    ...
    constexpr std::true_type try_parse(Reader reader) {
      while (true) {
        ...
        // This is a potential optimization: as we're accepting EOF anyway, we
        // don't need to enter Condition's parsing logic.
...
template <typename Condition> struct _until : token_base<_until<Condition>> {
  template <typename Reader> struct tp {
    typename Reader::marker end;
    ...
    constexpr bool try_parse(Reader reader) {
      while (true) {
        ...
        // We need to do it after checking for condition, as the condition
        // might just accept EOF.
        if (reader.peek() == Reader::encoding::eof()) {
  ...
  constexpr auto or_eof() const { return _until_eof<Condition>{}; }
};

/// Matches anything until Condition matches.
/// Then matches Condition.
template <typename Condition> constexpr auto until(Condition) {
...
namespace lexy {
template <typename Condition>
constexpr auto token_kind_of<lexy::dsl::_until_eof<Condition>> =
    lexy::any_token_kind;
template <typename Condition>
constexpr auto token_kind_of<lexy::dsl::_until<Condition>> =
    lexy::any_token_kind;
} // namespace lexy
