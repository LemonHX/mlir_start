...
namespace lexyd {
template <typename Condition, typename... R> struct _br;
template <typename... R> struct _seq_impl;
} // namespace lexyd

//=== tag type ===//
namespace lexy {
template <typename Literal> struct _op {};
// GCC is buggy with auto parameters.
template <typename T, T Value> struct _opv {
  constexpr operator LEXY_DECAY_DECLTYPE(Value)() const { return Value; }
};
...
template <typename... Literals> struct op_lit_list {
  ...
  template <typename... T> constexpr auto operator+(op_lit_list<T...>) const {
    return op_lit_list<Literals..., T...>{};
  }
};

template <typename Reader> struct parsed_operator {
...
constexpr auto parse_operator(Reader &reader) {
  ...
  using op_matcher =
      lexy::_detail::lit_trie_matcher<OpList::template trie<encoding>, 0>;
...
struct _op : branch_base {
  ...
  template <typename NextParser, typename Context, typename Reader,
            typename... Args>
  LEXY_PARSER_FUNC static bool
  op_finish(Context &context, Reader &reader,
            lexy::_detail::parsed_operator<Reader> op, Args &&...args) {
    context.on(_ev::token{}, typename Literal::token_type{}, op.cur.position(),
               reader.position());

    using continuation =
        lexy::whitespace_parser<Context,
                                lexy::parser_for<_seq_impl<R...>, NextParser>>;
    if constexpr (std::is_void_v<TagType>)
      return continuation::parse(context, reader, LEXY_FWD(args)...);
    else if constexpr (lexy::_detail::is_detected<
                           _detect_op_tag_ctor_with_state, op_tag_type, Reader,
                           Context>)
      return continuation::parse(
          context, reader, LEXY_FWD(args)...,
          op_tag_type(*context.control_block->parse_state, op.pos));
    else if constexpr (lexy::_detail::is_detected<_detect_op_tag_ctor,
                                                  op_tag_type, Reader>)
      return continuation::parse(context, reader, LEXY_FWD(args)...,
  ...
  template <typename Reader> struct bp {
    ...
    template <typename Context> constexpr void cancel(Context &context) {
      impl.cancel(context);
    }
    ...
    LEXY_PARSER_FUNC bool finish(Context &context, Reader &reader,
      ...
      if constexpr (std::is_void_v<TagType>)
        return impl.template finish<continuation>(context, reader,
                                                  LEXY_FWD(args)...);
      else if constexpr (lexy::_detail::is_detected<
                             _detect_op_tag_ctor_with_state, op_tag_type,
                             Reader, Context>)
        return impl.template finish<continuation>(
            context, reader, LEXY_FWD(args)...,
            op_tag_type(*context.control_block->parse_state,
                        reader.position()));
                                                    ...
                                                    op_tag_type, Reader>)
        return impl.template finish<continuation>(
            context, reader, LEXY_FWD(args)..., op_tag_type(reader.position()));
      else
        return impl.template finish<continuation>(
            context, reader, LEXY_FWD(args)..., op_tag_type{});
    }
  };
...
template <typename Literal> constexpr auto op(Literal) {
...
template <typename Tag, typename Literal> constexpr auto op(Literal) {
  static_assert(lexy::is_literal_rule<Literal>);
...
template <auto Tag, typename Literal> constexpr auto op(Literal) {
  static_assert(lexy::is_literal_rule<Literal>);
...
//=== op choice ===//
namespace lexyd {
template <typename... Ops> struct _opc : branch_base {
  ...
  op_finish(Context &context, Reader &reader,
    ...
    (void)((cur_idx == op.idx ? (result = Ops::template op_finish<NextParser>(
                                     context, reader, op, LEXY_FWD(args)...),
                                 true)
                              : (++cur_idx, false)) ||
           ...);
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      if (!impl.try_parse(context.control_block, reader)) {
        auto err = lexy::error<Reader, lexy::expected_literal_set>(
            impl.op.cur.position());
...
template <typename T1, typename L1, typename... R1, typename T2, typename L2,
          typename... R2>
