...
#include "lib/lexy/action/base.hpp"
#include "lib/lexy/dsl/base.hpp"
#include "lib/lexy/dsl/literal.hpp"
#include "lib/lexy/dsl/operator.hpp"

// Expression parsing algorithm uses an adapted version of Pratt parsing, as
// described here:
...
struct atom : _operation_base {
  static LEXY_CONSTEVAL auto name() { return "atom"; }
};

/// Operation that selects between multiple ones.
template <typename... Operands> struct groups : _operation_base {};
...
struct binding_power {
  ...
  constexpr bool is_valid() const { return lhs > 0 || rhs > 0; }
  constexpr bool is_infix() const { return lhs > 0 && rhs > 0; }
  constexpr bool is_postfix() const { return lhs > 0 && rhs == 0; }
  constexpr bool is_prefix() const { return lhs == 0 && rhs > 0; }
};

template <typename Operation>
constexpr binding_power get_binding_power(unsigned cur_group,
                                          unsigned cur_level) {
  if constexpr (std::is_base_of_v<lexyd::infix_op_left, Operation>
                // We treat a list as left associative operator for simplicity
                // here. It doesn't really matter, as it will only consider
                // operators from the same operation anyway.
                || std::is_base_of_v<lexyd::infix_op_list, Operation>
                // For the purposes of error recovery, single is left
                // associative.
...
template <typename DestOperation> struct _binding_power_of {
  ...
  static constexpr auto transition(CurOperation op, unsigned cur_group,
                                   unsigned cur_level) {
    // Normal operation: keep group the same, but increment level.
    return get(op, cur_group, cur_level + 1);
  }
  template <typename... Operations>
  static constexpr auto transition(lexyd::groups<Operations...>,
                                   unsigned cur_group, unsigned cur_level) {
    ...
    // Note that we don't increment the level, as that is handled by the
    // overload above.
  ...
  template <typename CurOperation>
  static constexpr auto get(CurOperation, unsigned cur_group,
                            unsigned cur_level) {
...
constexpr auto binding_power_of(Operation) {
  return _binding_power_of<Operation>::transition(typename Expr::operation{}, 0,
                                                  0);
}
} // namespace lexy::_detail

namespace lexy::_detail {
template <typename Operation> using op_of = LEXY_DECAY_DECLTYPE(Operation::op);

template <typename... Operations> struct operation_list {
  ...
  using ops = decltype((typename op_of<Operations>::op_literals{} + ... +
                        op_lit_list{}));

  template <template <typename> typename Continuation, typename Context,
            typename Reader, typename... Args>
  static constexpr bool apply(Context &context, Reader &reader,
                              parsed_operator<Reader> op, Args &&...args) {
    auto result = false;

    auto cur_idx = std::size_t(0);
    (void)((cur_idx <= op.idx &&
                    op.idx < cur_idx + op_of<Operations>::op_literals::size
                ? (result = Continuation<Operations>::parse(
                       context, reader,
                       parsed_operator<Reader>{op.cur, op.idx - cur_idx},
                       LEXY_FWD(args)...),
                   true)
...
template <bool Pre, unsigned MinBindingPower> struct _operation_list_of {
  template <unsigned CurLevel> static constexpr auto get(lexyd::atom) {
...
using pre_operation_list_of =
    decltype(_operation_list_of<true, MinBindingPower>::template get<1>(
        typename Expr::operation{}));

// infix and postfix operations
template <typename Expr, unsigned MinBindingPower>
using post_operation_list_of =
    decltype(_operation_list_of<false, MinBindingPower>::template get<1>(
        typename Expr::operation{}));
} // namespace lexy::_detail

//=== expression rule ===//
namespace lexyd {
template <typename RootOperation> struct _expr : rule_base {
  struct _state {
    unsigned cur_group = 0;
    unsigned cur_nesting_level = 0;
  };

  template <typename Operation> struct _continuation {
    struct _op_cont {
      template <typename Context, typename Reader, typename... Args>
      LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
                                         _state &state, Args &&...op_args) {
        using namespace lexy::_detail;

        constexpr auto value_type_void =
            std::is_void_v<typename Context::value_type>;
        ...
        if constexpr (std::is_base_of_v<infix_op_list, Operation>) {
          ...
          while (true) {
            ...
            if (!op_rule::template op_finish<lexy::sink_parser>(context, reader,
                                                                op, sink)) {
              result = false;
              break;
            }
          ...
          if constexpr (std::is_base_of_v<infix_op_single, Operation>) {
            ...
            if (op.idx < op_rule::op_literals::size) {
              ...
              auto err = lexy::error<Reader, tag>(op.cur.position(),
                                                  reader.position());
    ...
    static constexpr bool parse(Context &context, Reader &reader,
      ...
      return op_of<Operation>::template op_finish<_op_cont>(context, reader, op,
                                                            state);
    }
  };

  template <unsigned MinBindingPower, typename Context, typename Reader>
  static constexpr bool _parse_lhs(Context &context, Reader &reader,
                                   _state &state) {
    using namespace lexy::_detail;

    using op_list =
        pre_operation_list_of<typename Context::production, MinBindingPower>;
    using atom_parser =
        lexy::parser_for<LEXY_DECAY_DECLTYPE(Context::production::atom),
                         final_parser>;
    ...
    if constexpr (op_list::size == 0) {
      ...
      auto start_event =
          context.on(_ev::operation_chain_start{}, op.cur.position());
      auto result =
          op_list::template apply<_continuation>(context, reader, op, state);
      context.on(_ev::operation_chain_finish{}, LEXY_MOV(start_event),
                 reader.position());
      return result;
    }
  }
  ...
  template <unsigned MinBindingPower, typename Context, typename Reader>
  static constexpr bool _parse(Context &context, Reader &reader,
                               _state &state) {
    using namespace lexy::_detail;
    using op_list =
        post_operation_list_of<typename Context::production, MinBindingPower>;
    ...
    if constexpr (op_list::size == 0) {
      ...
      auto start_event =
          context.on(_ev::operation_chain_start{}, reader.position());
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader) {
      ...
      using production = typename Context::production;
      constexpr auto binding_power =
          lexy::_detail::binding_power_of<production>(
              lexy::_detail::type_or<RootOperation,
                                     typename production::operation>{});
      // The MinBindingPower is determined by the root operation.
      // The initial operand is always on the left, so we use the left binding
      // power. However, for a prefix operator it is zero, but then it's a right
      // operand so we use that.
      ...
      // Regardless of parse errors, we can recover if we already had a value at
      // some point.
...
struct operator_chain_error {
  static LEXY_CONSTEVAL auto name() { return "operator cannot be chained"; }
};
