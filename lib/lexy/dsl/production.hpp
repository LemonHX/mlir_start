...
namespace lexyd {
/// Parses the rule of the production as if it were part of the current
/// production.
...
/* not force inline */ constexpr bool _parse_production(Context &context,
                                                        Reader &reader) {
  using parser = lexy::parser_for<lexy::production_rule<Production>,
...
/* not force inline */ constexpr bool
_finish_production(ProductionParser &parser, Context &context, Reader &reader) {
...
struct _prd
    ...
    : std::conditional_t<lexy::_production_defines_whitespace<Production>,
                         rule_base,
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      if constexpr (lexy::_production_defines_whitespace<Production>) {
        if (!lexy::whitespace_parser<decltype(sub_context),
                                     lexy::pattern_parser<>>::parse(sub_context,
                                                                    reader)) {
      ...
      if (_parse_production<Production>(sub_context, reader)) {
        ...
        return continuation::parse(context, reader, sub_context,
                                   LEXY_FWD(args)...);
      } else {
        // Cancel.
        sub_context.on(_ev::production_cancel{}, reader.position());
...
template <typename Production> constexpr auto p = _prd<Production>{};
} // namespace lexyd
...
struct _recb : branch_base {
  template <typename NextParser> struct _depth_handler {
    ...
    static constexpr bool increment_depth(Context &context, Reader &reader) {
      ...
      if (control_block->cur_depth >= control_block->max_depth) {
        ...
        using tag = lexy::_detail::type_or<DepthError,
  ...
  template <typename Reader> struct bp {
    LEXY_REQUIRE_BRANCH_RULE(lexy::production_rule<Production>,
                             "recurse_branch");
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      return lexy::parser_for<_prd<Production>, depth>::parse(
...
/// `dsl::p` requires that the production is already defined in order to
/// propagate a branch condition outwards.
template <typename Production> constexpr auto recurse = _rec<Production>{};
