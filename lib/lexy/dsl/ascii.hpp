...
// SWAR tricks inspired by
// https://garbagecollected.org/2017/01/31/four-column-ascii/.

namespace lexyd::ascii {
//=== control ===//
struct _control : char_class_base<_control> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.control"; }
...
struct _blank : char_class_base<_blank> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.blank"; }
...
struct _newline : char_class_base<_newline> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.newline"; }
...
struct _other_space : char_class_base<_other_space> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.other-space"; }
...
struct _space : char_class_base<_space> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.space"; }
...
struct _lower : char_class_base<_lower> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.lower"; }
  ...
  static constexpr auto char_class_match_swar(lexy::_detail::swar_int c) {
    ...
    return ((c - offset_low) & mask) == expected &&
           ((c + offset_high) & mask) == expected;
  }
};
inline constexpr auto lower = _lower{};

struct _upper : char_class_base<_upper> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.upper"; }
...
struct _alpha : char_class_base<_alpha> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.alpha"; }
  ...
  template <typename Encoding>
  static constexpr auto char_class_match_swar(lexy::_detail::swar_int c) {
    // We're assuming lower characters are more common, so do the efficient
    // check only for them.
...
struct _alphau : char_class_base<_alphau> {
  ...
  template <typename Encoding>
  static constexpr auto char_class_match_swar(lexy::_detail::swar_int c) {
    // We're assuming alpha characters are more common, so do the efficient
    // check only for them.
...
struct _digit : char_class_base<_digit> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.digit"; }
...
struct _alnum : char_class_base<_alnum> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.alpha-digit"; }
...
struct _word : char_class_base<_word> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.word"; }
  ...
  template <typename Encoding>
  static constexpr auto char_class_match_swar(lexy::_detail::swar_int c) {
    // We're assuming alphau characters are more common, so do the efficient
    // check only for them.
...
struct _punct : char_class_base<_punct> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.punct"; }
...
struct _graph : char_class_base<_graph> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.graph"; }
  ...
  static constexpr auto char_class_match_swar(lexy::_detail::swar_int c) {
    ...
    // First check that we have only ASCII, but shifted by one, so we also
    // exclude 0x7F.
    constexpr auto ascii_mask =
        lexy::_detail::swar_fill_compl(char_type(0b11'11111));
    ...
    // The above check also included 0xFF for single byte encodings where it
    // overflowed, so do a separate check in those cases.
    ...
    // If we subtract one we turn 0x21-0x01 into column 0 and 0x00 to a value
    // definitely not in column 0, so need to check both.
    constexpr auto mask = lexy::_detail::swar_fill_compl(char_type(0b11111));
    constexpr auto offset_low = lexy::_detail::swar_fill(char_type(1));
...
struct _print : char_class_base<_print> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII.print"; }
...
struct _char : char_class_base<_char> {
  static LEXY_CONSTEVAL auto char_class_name() { return "ASCII"; }
...
namespace lexyd::ascii {
template <char... C> struct _alt : char_class_base<_alt<C...>> {
...
template <typename CharT, CharT... C> struct _one_of {
  static_assert((std::is_same_v<CharT, char> && ... &&
                 lexy::_detail::is_ascii(C)),
...
constexpr auto one_of =
    typename lexy::_detail::to_type_string<_one_of, Str>::rule{};
#endif
