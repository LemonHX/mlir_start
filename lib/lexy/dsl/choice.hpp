...
struct exhausted_choice {
  static LEXY_CONSTEVAL auto name() { return "exhausted choice"; }
};
} // namespace lexy
...
struct _chc
    ...
    // A choice rule with an unconditional branch is itself an unconditional
    // branch, which is most likely a bug.
    : std::conditional_t<(lexy::is_unconditional_branch_rule<R> || ...),
                         rule_base, branch_base> {
  static constexpr auto _any_unconditional =
      (lexy::is_unconditional_branch_rule<R> || ...);
  ...
  struct bp<Reader, lexy::_detail::index_sequence<Idx...>> {
    template <typename Rule> using rp = lexy::branch_parser_for<Rule, Reader>;
    ...
    LEXY_PARSER_FUNC bool finish(Context &context, Reader &reader,
                                 Args &&...args) {
      // Need to call finish on the selected branch, and cancel on all others
      // before that.
      auto result = false;
      (void)((Idx == branch_idx
                  ? (result = r_parsers.template get<Idx>()
                                  .template finish<NextParser>(
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      auto try_r = [&](auto &&parser) {
        ...
        result = parser.template finish<NextParser>(
            context, reader, static_cast<Args &&>(args)...);
        return true;
      };
      ...
      if constexpr (_any_unconditional) {
        ...
        auto err =
            lexy::error<Reader, lexy::exhausted_choice>(reader.position());
...
template <typename R, typename S> constexpr auto operator|(R, S) {
  LEXY_REQUIRE_BRANCH_RULE(R, "choice");
  LEXY_REQUIRE_BRANCH_RULE(S, "choice");
  return _chc<R, S>{};
}
template <typename... R, typename S> constexpr auto operator|(_chc<R...>, S) {
  LEXY_REQUIRE_BRANCH_RULE(S, "choice");
  return _chc<R..., S>{};
}
template <typename R, typename... S> constexpr auto operator|(R, _chc<S...>) {
  LEXY_REQUIRE_BRANCH_RULE(R, "choice");
  return _chc<R, S...>{};
}
