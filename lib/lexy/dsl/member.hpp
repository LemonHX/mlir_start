...
#include "lib/lexy/_detail/stateless_lambda.hpp"
#include "lib/lexy/dsl/base.hpp"
#include "lib/lexy/dsl/branch.hpp"

namespace lexy {
template <auto Ptr> struct _mem_ptr_fn {
...
template <typename Fn> struct member {};

template <auto Ptr> using make_member_ptr = member<_mem_ptr_fn<Ptr>>;
} // namespace lexy

namespace lexyd {
template <typename Fn, typename Rule> struct _mem : _copy_base<Rule> {
  template <typename Reader> struct bp {
    ...
    LEXY_PARSER_FUNC auto finish(Context &context, Reader &reader,
                                 Args &&...args) {
      // Add member tag here.
      return rule.template finish<NextParser>(
          context, reader, LEXY_FWD(args)..., lexy::member<Fn>{});
    }
  };
...
template <typename Fn> struct _mem_dsl {
  constexpr _mem_dsl(Fn = {}) {}

  template <typename Rule>
  constexpr auto
  operator=(Rule) const // NOLINT: it _is_ an unconventional assignment operator
  {
...
/// Specifies that the output of the associated rule should be stored in the
/// member pointer. Used with `lexy::as_aggregate`.
