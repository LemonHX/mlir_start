...
template <typename Rule> struct _posr : _copy_base<Rule> {
  template <typename Reader> struct bp {
    ...
    LEXY_PARSER_FUNC auto finish(Context &context, Reader &reader,
      ...
      auto pos = reader.position();
      context.on(_ev::token{}, lexy::position_token_kind, pos, pos);
      return rule.template finish<NextParser>(context, reader,
                                              LEXY_FWD(args)..., pos);
    }
  };
...
/// Produces an iterator to the current reader position without parsing
/// anything.
