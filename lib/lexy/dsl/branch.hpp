...
struct _br : _copy_base<Condition> {
  ...
  // Condition's try_parse() checks the branch condition, which is what we
  // want.
...
// If we add two branches, we use the condition of the first one and treat the
// second as sequence.
template <typename C1, typename... R, typename C2, typename... S>
constexpr auto operator+(_br<C1, R...>, _br<C2, S...>) {
  return _br<C1, R..., C2, S...>{};
...
struct _else_dsl {
  template <typename R> friend constexpr auto operator>>(_else_dsl, R rule) {
    return _else{} >> rule;
  }
  template <typename... R>
