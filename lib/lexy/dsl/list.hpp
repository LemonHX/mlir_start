...
namespace lexyd {
template <typename Item, typename Sep> struct _lst : _copy_base<Item> {
  template <typename Context, typename Reader, typename Sink>
  LEXY_PARSER_FUNC static bool _loop(Context &context, Reader &reader,
                                     Sink &sink) {
    while (true) {
      ...
      if constexpr (lexy::is_branch_rule<Item>) {
        ...
        if (!lexy::parser_for<Item, lexy::sink_parser>::parse(context, reader,
                                                              sink))
          return false;
      }
    }
...
template <typename Item> constexpr auto list(Item) {
...
template <typename Item, typename Sep> constexpr auto list(Item, _tsep<Sep>) {
  LEXY_REQUIRE_BRANCH_RULE(Item, "list() with a trailing separator");
  return _lst<Item, _tsep<Sep>>{};
}

template <typename Item, typename Sep> constexpr auto list(Item, _isep<Sep>) {
...
struct _lstt : rule_base {
  // We're using an enum together with a switch to compensate a lack of goto in
  // constexpr. The simple state machine goes as follows on well-formed input:
  // terminator -> separator -> separator_trailing_check -> item -> terminator
  // -> ... -> done
  //
  // The interesting case is error recovery.
  // There we skip over characters until we either found the terminator,
  // separator or item. We then set the enum to jump to the appropriate state of
  // the state machine.
  enum class _state {
    terminator,
    separator,
  ...
  template <typename TermParser, typename Context, typename Reader,
            typename Sink>
  LEXY_PARSER_FUNC static bool _loop(_state initial_state, TermParser &term,
                                     Context &context, Reader &reader,
                                     Sink &sink) {
    ...
    while (true) {
      ...
      case _state::separator:
        if constexpr (!std::is_void_v<Sep>) {
          ...
          if (lexy::parser_for<typename Sep::rule, lexy::sink_parser>::parse(
                  context, reader, sink)) {
            ...
            if constexpr (lexy::is_branch_rule<Item>) {
              lexy::branch_parser_for<Item, Reader> item{};
              if (item.try_parse(context.control_block, reader) &&
                  item.template finish<lexy::sink_parser>(context, reader,
                                                          sink)) {
            ...
            // separator share a common prefix, we know it wasn't the start of
            // an item so can't just pretend that there is one).
            state = _state::recovery;
            break;
          }
        } else {
      ...
      case _state::separator_trailing_check:
        if constexpr (!std::is_void_v<Sep>) {
          // We need to check whether we're having a trailing separator by
          // checking for a terminating one.
          if (term.try_parse(context.control_block, reader)) {
            // We had the terminator, so the list is done.
        ...
        while (true) {
          ...
          if constexpr (!std::is_void_v<Sep>) {
            ...
            if (sep.try_parse(context.control_block, reader)) {
              ...
              if (sep.template finish<lexy::sink_parser>(context, reader,
                                                         sink)) {
          ...
          // When we don't have a separator, but the item is a branch, we also
          // succeed when we reach the next item.
          //
          // Note that we're doing this check only if we don't have a separator.
          // If we do have one, the heuristic "end of the invalid item" is
          // better than "beginning of the next one".
            ...
            if (item.try_parse(context.control_block, reader)) {
              ...
              if (item.template finish<lexy::sink_parser>(context, reader,
                                                          sink)) {
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      // Parse the remaining items.
      if (!_loop(result ? _state::terminator : _state::recovery, term, context,
                 reader, sink))
        return false;
      ...
      if constexpr (std::is_same_v<typename decltype(sink)::return_type,
        ...
        return term.template finish<NextParser>(context, reader,
                                                LEXY_FWD(args)...);
      } else {
        return term.template finish<NextParser>(
