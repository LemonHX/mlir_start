...
namespace lexyd {
template <typename Rule> struct _wsr;
}

//=== implementation ===//
namespace lexy::_detail {
// Parse this production to skip whitespace.
template <typename WhitespaceRule> struct ws_production {
  static constexpr auto name = "<whitespace>";
  static constexpr auto max_recursion_depth = 0;
  static constexpr auto rule =
      lexy::dsl::loop(WhitespaceRule{} | lexy::dsl::break_);
};
// If the whitespace rule itself uses `dsl::whitespace`, strip it to avoid
// infinite recursion.
...
template <typename Handler> class ws_handler {
public:
  constexpr explicit ws_handler(Handler &handler,
  ...
  class event_handler {
  public:
    template <typename Rule> constexpr event_handler(ws_production<Rule>) {}
    template <typename Production> constexpr event_handler(Production) {
      static_assert(
          _detail::error<Production>,
          "whitespace rule must not contain `dsl::p` or `dsl::recurse`;"
          "use `dsl::inline_` instead");
    }

    template <typename Error>
    constexpr void on(ws_handler &handler, parse_events::error ev,
                      const Error &error) {
  ...
  template <typename> constexpr bool get_result(bool rule_parse_result) && {
...
template <typename> using ws_result = bool;
...
constexpr auto skip_whitespace(ws_handler<Handler> &&handler, Reader &reader) {
  ...
  if constexpr (lexy::is_token_rule<WhitespaceRule>) {
    ...
    handler.real_on(lexy::parse_events::token{}, lexy::whitespace_token_kind,
    ...
    auto result = lexy::do_action<production, ws_result>(
        LEXY_MOV(handler), lexy::no_parse_state, reader);

    handler.real_on(lexy::parse_events::token{},
                    result ? lexy::whitespace_token_kind
...
template <typename NextParser> struct automatic_ws_parser {
  ...
  LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
    ...
    if (!std::is_base_of_v<disable_whitespace_skipping, NextParser> //
      ...
      using whitespace =
          lexy::production_whitespace<typename Context::production,
...
//=== whitespace ===//
namespace lexyd {
template <typename Rule> struct _wsr : rule_base {
  template <typename NextParser> struct p {
    template <typename Context, typename Reader, typename... Args>
    constexpr static bool parse(Context &context, Reader &reader,
                                Args &&...args) {
      auto result = lexy::_detail::skip_whitespace<Rule>(
          lexy::_detail::ws_handler(context), reader);
      if (!result)
        return false;
  ...
  template <typename R> friend constexpr auto operator|(_wsr<Rule>, R r) {
    return _wsr<decltype(Rule{} | r)>{};
  }
  template <typename R> friend constexpr auto operator|(R r, _wsr<Rule>) {
    return _wsr<decltype(r | Rule{})>{};
  }
};

template <typename Rule> constexpr auto whitespace(Rule) {
  return _wsr<Rule>{};
}
} // namespace lexyd
...
//=== no_whitespace ===//
namespace lexyd {
template <typename Rule> struct _wsn : _copy_base<Rule> {
  ...
  template <typename Reader> struct bp {
    ...
    constexpr auto try_parse(const ControlBlock *cb, const Reader &reader) {
      // Note that this can't skip whitespace as there is no way to access the
      // whitespace rule. We thus don't need to disable anything.
    ...
    LEXY_PARSER_FUNC auto finish(Context &context, Reader &reader,
                                 Args &&...args) {
      // Finish the rule with whitespace skipping disabled.
      context.control_block->enable_whitespace_skipping = false;
      return rule.template finish<_pc<NextParser>>(context, reader,
                                                   LEXY_FWD(args)...);
    }
  };
...
template <typename Rule> constexpr auto no_whitespace(Rule) {
