...
// Parses the rule but generates the appropriate recovery_start/finish/cancel
// events.
template <typename Rule> struct _recovery_wrapper : _recovery_base {
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      using parser =
          lexy::parser_for<Rule,
                           lexy::whitespace_parser<Context, _continuation>>;
      auto result =
          parser::parse(context, reader, recovery_finished, LEXY_FWD(args)...);
...
struct _noop_recovery : rule_base {
  template <typename NextParser> using p = NextParser;
};
} // namespace lexyd

namespace lexyd {
template <typename Token, typename Limit> struct _find : _recovery_base {
  template <typename NextParser> struct p {
    template <typename Context, typename Reader, typename... Args>
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
                                       Args &&...args) {
      // Note that we can't use lookahead() directly as it's end position
      // includes Needle/End, here we want to exclude it, however.
      ...
      while (true) {
        ...
        if (result == 0) {
          context.on(_ev::token{}, lexy::error_token_kind, begin,
                     end.position());
...
/// Recovers once it finds one of the given literal tokens (without consuming
/// them).
template <typename... Literals> constexpr auto find(Literals... literals) {
  static_assert(sizeof...(Literals) > 0);
...
namespace lexyd {
template <typename Limit, typename... R> struct _reco : _recovery_base {
  template <typename NextParser> struct p {
    template <typename Context, typename Reader, typename... Args>
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
                                       Args &&...args) {
      ...
      return recovery.template finish<NextParser>(context, reader,
                                                  LEXY_FWD(args)...);
    }
  };
...
template <typename... Branches> constexpr auto recover(Branches...) {
...
struct _tryt : rule_base {
  template <typename NextParser> struct _pc {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      if constexpr (std::is_void_v<Terminator>)
        ...
        return lexy::parser_for<Terminator, NextParser>::parse(
    ...
    LEXY_PARSER_FUNC static bool recover(Context &context, Reader &reader,
      ...
      if constexpr (std::is_void_v<Recover>) {
        ...
        return lexy::parser_for<recovery_rule, NextParser>::parse(
            context, reader, LEXY_FWD(args)...);
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      auto result = parser::parse(context, reader, continuation_reached,
                                  LEXY_FWD(args)...);
      if (continuation_reached)
        // Whatever happened, it is not our problem as we've reached the
        // continuation.
        return result;
...
template <typename Rule, typename Recover> struct _tryr : _copy_base<Rule> {
  ...
  template <typename Reader> struct bp {
    ...
    LEXY_PARSER_FUNC bool finish(Context &context, Reader &reader,
      ...
      auto continuation_reached = false;
      auto result = rule.template finish<continuation>(
          context, reader, continuation_reached, LEXY_FWD(args)...);
      if (continuation_reached)
        // Whatever happened, it is not our problem as we've reached the
        // continuation.
        return result;
...
template <typename Rule> constexpr auto try_(Rule) {
  return _tryr<Rule, void>{};
}

/// Parses Rule, if that fails, parses recovery rule.
template <typename Rule, typename Recover> constexpr auto try_(Rule, Recover) {
