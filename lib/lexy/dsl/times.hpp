...
struct _times : rule_base {
  template <std::size_t I = N> static constexpr auto _repeated_rule() {
    if constexpr (I == 1) {
...
template <std::size_t N, typename Rule> constexpr auto times(Rule) {
  static_assert(N > 0);
...
template <typename Rule> constexpr auto twice(Rule rule) {
  return times<2>(rule);
}
