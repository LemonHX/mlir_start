...
#include "lib/lexy/callback/object.hpp"
#include "lib/lexy/dsl/base.hpp"

namespace lexyd {
// Custom handler that forwards events but overrides the value callback.
template <typename Handler> struct _pas_handler {
  Handler &_handler;

  using event_handler = typename Handler::event_handler;

  // We are implicitly convertible to all handler types the original handler
  // is convertible to. This is because the handler is passed to
  // event_handler::on.
  template <typename H,
            typename = decltype(static_cast<H &>(LEXY_DECLVAL(Handler &)))>
  constexpr operator H &() const {
    return static_cast<H &>(_handler);
  }

  // We use ::value to get a value.
  // We can't use it unconditionally, as the initial production that contains
  // the parse_as might not have one. So we silently fallback if that's the
  // case
  // - this might cause worse errors if the value is missing.
...
template <typename Handler> constexpr auto _make_pas_handler(Handler &handler) {
...
struct _pas : _copy_base<Rule> {
  template <typename Reader> struct bp {
    ...
    template <typename Context> constexpr void cancel(Context &context) {
      // No need to use the special context here; it doesn't produce any
      // values.
    ...
    LEXY_PARSER_FUNC bool finish(Context &context, Reader &reader,
      ...
      lexy::_detail::parse_context_control_block cb(LEXY_MOV(handler),
      ...
      lexy::_detail::lazy_init<T> value;
      auto result = rule_parser.template finish<_pas_final_parser>(
          sub_context, reader, value);
      ...
      if (!result)
        ...
        return NextParser::parse(context, reader, *LEXY_MOV(value),
                                 LEXY_FWD(args)...);
      else
        return NextParser::parse(context, reader, LEXY_FWD(args)...,
                                 *LEXY_MOV(value));
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      lexy::_detail::lazy_init<T> value;
      auto result = lexy::parser_for<Rule, _pas_final_parser>::parse(
          sub_context, reader, value);
...
template <typename T, typename Rule> constexpr auto parse_as(Rule) {
  return _pas<T, Rule>{};
}
} // namespace lexyd
