...
public:
  ...
  LEXY_CONSTEVAL auto case_folding(CaseFoldingDSL) const {
    return _symbol_table<T, CaseFoldingDSL::template case_folding, Strings...>(
        _detail::make_index_sequence<size()>{}, *this);
  }
  ...
  LEXY_CONSTEVAL auto map(Args &&...args) const {
    ...
    if constexpr (empty())
      return next_table(_detail::make_index_sequence<0>{}, nullptr,
                        LEXY_FWD(args)...);
    else
      return next_table(_detail::make_index_sequence<size()>{}, _data,
                        LEXY_FWD(args)...);
  }
  ...
  LEXY_CONSTEVAL auto map(Args &&...args) const {
    return map<_detail::to_type_string<_detail::type_string, SymbolString>>(
        LEXY_FWD(args)...);
  }
#else
...
#endif
  LEXY_CONSTEVAL auto map(Args &&...args) const {
    return map<_detail::type_string<LEXY_DECAY_DECLTYPE(C), C>>(
        LEXY_FWD(args)...);
  }
#endif
  ...
  class iterator
      : public lexy::_detail::bidirectional_iterator_base<iterator, value_type,
                                                          value_type, void> {
  public:
    ...
    constexpr value_type deref() const noexcept {
      if constexpr (empty()) {
        ...
        constexpr const char_type *strings[] = {
            Strings::template c_str<char_type>...};
  ...
  constexpr key_index try_parse(Reader &reader) const {
    ...
    auto result =
        _detail::lit_trie_matcher<_trie<encoding>, 0>::try_match(reader);
...
private:
  ...
  static constexpr lexy::_detail::lit_trie<Encoding, CaseFolding,
                                           _max_char_count>
      _trie = _build_trie<Encoding>();

  template <std::size_t... Idx, typename... Args>
  constexpr explicit _symbol_table(lexy::_detail::index_sequence<Idx...>,
                                   const T *data, Args &&...args)
  ...
  constexpr explicit _symbol_table(
      lexy::_detail::index_sequence<Idx...>,
...
struct unknown_symbol {
  static LEXY_CONSTEVAL auto name() { return "unknown symbol"; }
};
} // namespace lexy

namespace lexyd {
template <typename Leading, typename Trailing> struct _idp;
template <typename Leading, typename Trailing, typename... Reserved> struct _id;
...
struct _sym : branch_base {
  template <typename Reader> struct bp {
    ...
    constexpr auto value() const { return Table[symbol]; }
  ...
  template <typename NextParser> struct p {
    template <typename... PrevArgs> struct _cont {
      ...
      LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
        ...
        auto content =
            lexy::partial_input(reader, lexeme.begin(), lexeme.end());
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      return lexy::parser_for<_cap<Token>, _cont<Args...>>::parse(
          context, reader, LEXY_FWD(args)...);
    }
  };
...
// Optimization for identifiers: instead of parsing an entire identifier (which
// requires checking every character against the char class), parse a symbol and
// check whether the next character would continue the identifier. This is the
// same optimization that is done for keywords.
  ...
  template <typename NextParser> struct p {
    ...
    LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
      ...
      if (impl.try_parse(context.control_block, reader))
        return impl.template finish<NextParser>(context, reader,
                                                LEXY_FWD(args)...);
      impl.cancel(context);
...
template <const auto &Table> struct _sym_dsl : _sym<Table, void, void> {
  template <typename Token> constexpr auto operator()(Token) const {
...
/// Parses optional rule, then matches the resulting lexeme against the symbol
/// table.
template <const auto &Table> constexpr auto symbol = _sym_dsl<Table>{};
} // namespace lexyd
