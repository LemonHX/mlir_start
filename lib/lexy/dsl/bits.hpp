...
template <unsigned Value> struct _n : _bit_rule {
...
template <unsigned Value> constexpr auto nibble = _n<Value>{};

template <unsigned N> struct _b : _bit_rule {
  static_assert(N > 0);
...
template <unsigned N> constexpr auto any = _b<N>{};
} // namespace lexyd::bit
...
template <typename... Bits> constexpr auto bits(Bits...) {
  static_assert((std::is_base_of_v<bit::_bit_rule, Bits> && ...),
