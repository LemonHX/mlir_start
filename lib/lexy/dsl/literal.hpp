...
#include "lib/lexy/_detail/code_point.hpp"
#include "lib/lexy/_detail/integer_sequence.hpp"
#include "lib/lexy/_detail/iterator.hpp"
#include "lib/lexy/_detail/nttp_string.hpp"
#include "lib/lexy/_detail/swar.hpp"
#include "lib/lexy/dsl/base.hpp"
#include "lib/lexy/dsl/token.hpp"

//=== lit_matcher ===//
namespace lexy::_detail {
template <std::size_t CurCharIndex, typename CharT, CharT... Cs,
          typename Reader>
constexpr auto match_literal(Reader &reader) {
  ...
  // We only use SWAR if the reader supports it and we have enough to fill at
  // least one.
  else if constexpr (is_swar_reader<Reader> &&
                     sizeof...(Cs) >= swar_length<char_type>) {
    // Try and pack as many characters into a swar as possible, starting at
    // the current index.
    constexpr auto pack =
        swar_pack<CurCharIndex>(transcode_char<char_type>(Cs)...);
    ...
    if ((reader.peek_swar() & pack.mask) == pack.value) {
      ...
      return bool(
          match_literal<CurCharIndex + pack.count, CharT, Cs...>(reader));
    } else {
      auto partial = swar_find_difference<CharT>(reader.peek_swar() & pack.mask,
                                                 pack.value);
...
//=== lit_trie ===//
namespace lexy::_detail {
template <typename Encoding, template <typename> typename CaseFolding,
          std::size_t MaxCharCount, typename... CharClasses>
struct lit_trie {
  using encoding = Encoding;
  using char_type = typename Encoding::char_type;

  template <typename Reader> using reader = CaseFolding<Reader>;
  ...
  LEXY_CONSTEVAL lit_trie()
      : node_count(1), node_value{}, node_char_class{}, transition_char{},
  ...
  LEXY_CONSTEVAL std::size_t insert(std::size_t from, CharT _c) {
    ...
    // In a tree, we're always having node_count - 1 transitions, so that's the
    // upper bound. Everytime we add a transition from a node, its transition
    // index is >= its node index. As such, we start looking at the node index.
...
template <typename... CharClasses> struct char_class_list {
  template <typename Encoding, template <typename> typename CaseFolding,
            std::size_t N>
...
template <> struct _merge_case_folding<void> {
  template <typename Reader> using case_folding = Reader;
};
...
struct _merge_case_folding<CurrentCaseFolding, H, T...>
    : _merge_case_folding<
          std::conditional_t<std::is_void_v<CurrentCaseFolding>,
...
template <typename Reader> using lit_no_case_fold = Reader;
...
LEXY_CONSTEVAL auto make_empty_trie() {
  ...
  return typename decltype(char_classes)::template trie_type<
      Encoding, _merge_case_folding<void, Literals...>::template case_folding,
      max_char_count>{};
}
template <typename Encoding, typename... Literals>
...
struct _node_char_class_impl<Idx, true, H, T...>
    : _node_char_class_impl<Idx - 1, true, T...> {};
template <std::size_t CharClassIdx, typename... CharClasses>
using _node_char_class =
    _node_char_class_impl<CharClassIdx, (CharClassIdx < sizeof...(CharClasses)),
                          CharClasses...>;

template <const auto &Trie, std::size_t CurNode> struct lit_trie_matcher;
template <typename Encoding, template <typename> typename CaseFolding,
          std::size_t N, typename... CharClasses,
...
struct lit_trie_matcher<Trie, CurNode> {
  ...
  LEXY_FORCE_INLINE static constexpr bool
  _try_transition(std::size_t &result, Reader &reader, IntT cur) {
    ...
    reader.bump();
    result =
        lit_trie_matcher<Trie, Trie.transition_to[TransIdx]>::try_match(reader);
    return true;
  }
  ...
  struct _impl;
  template <std::size_t... Idx> struct _impl<index_sequence<Idx...>> {
    template <typename Reader>
    LEXY_FORCE_INLINE static constexpr std::size_t try_match(Reader &reader) {
      ...
      if constexpr (sizeof...(Idx) > 0) {
        ...
        (void)(_try_transition<transitions.index[Idx]>(next_value, reader,
                                                       cur_char) ||
               ...);
      ...
      // But first, we might need to check that we don't match that nodes char
      // class.
      constexpr auto char_class = Trie.node_char_class[CurNode];
      if constexpr (cur_value == Trie.node_no_match ||
                    char_class >= sizeof...(CharClasses)) {
        return cur_value;
      } else {
...
struct _lit
  ...
  static constexpr auto lit_first_char() -> typename Encoding::char_type {
    ...
    (void)((result = lexy::_detail::transcode_char<decltype(result)>(C),
            true) ||
           ...);
    return result;
  }
  ...
  template <typename Reader> struct tp {
    ...
    constexpr void report_error(Context &context, const Reader &reader) {
      using char_type = typename Reader::encoding::char_type;
      constexpr auto str =
          lexy::_detail::type_string<CharT, C...>::template c_str<char_type>;
...
template <auto C> constexpr auto lit_c = _lit<LEXY_DECAY_DECLTYPE(C), C>{};
...
namespace lexy {
template <typename CharT, CharT... C>
inline constexpr auto token_kind_of<lexy::dsl::_lit<CharT, C...>> =
    lexy::literal_token_kind;
} // namespace lexy

//=== lit_cp ===//
namespace lexyd {
template <char32_t... Cp> struct _lcp : token_base<_lcp<Cp...>>, _lit_base {
  template <typename Encoding> struct _string_t {
    typename Encoding::char_type data[4 * sizeof...(Cp)];
    std::size_t length = 0;

    constexpr _string_t() : data{} {
      ((length +=
        lexy::_detail::encode_code_point<Encoding>(Cp, data + length, 4)),
       ...);
    }
  };
  template <typename Encoding>
  ...
  template <typename Reader,
            typename Indices = lexy::_detail::make_index_sequence<
                _string<typename Reader::encoding>.length>>
  struct tp;
  ...
  struct tp<Reader, lexy::_detail::index_sequence<Idx...>> {
    ...
    constexpr bool try_parse(Reader reader) {
      ...
      auto result =
          lexy::_detail::match_literal<0, typename encoding::char_type,
    ...
    constexpr void report_error(Context &context, const Reader &reader) {
      ...
      auto begin = reader.position();
      auto index = lexy::_detail::range_size(begin, end.position());
      auto err = lexy::error<Reader, lexy::expected_literal>(
          begin, _string<encoding>.data, index, _string<encoding>.length);
      context.on(_ev::error{}, err);
    }
  };
};

template <char32_t... CodePoint> constexpr auto lit_cp = _lcp<CodePoint...>{};
} // namespace lexyd
...
//=== lit_set ===//
namespace lexy {
template <typename T, template <typename> typename CaseFolding,
          typename... Strings>
class _symbol_table;

struct expected_literal_set {
  static LEXY_CONSTEVAL auto name() { return "expected literal set"; }
};
} // namespace lexy
...
struct _lset : token_base<_lset<Literals...>>, _lset_base {
  ...
  template <typename Encoding> static LEXY_CONSTEVAL auto _build_trie() {
  ...
  template <typename Reader> struct tp {
    ...
    constexpr void report_error(Context &context, const Reader &reader) {
      auto err =
          lexy::error<Reader, lexy::expected_literal_set>(reader.position());
      context.on(_ev::error{}, err);
    }
  };

  //=== dsl ===//
  template <typename Lit> constexpr auto operator/(Lit) const {
  ...
  template <typename... Lit> constexpr auto operator/(_lset<Lit...>) const {
    return _lset<Literals..., Lit...>{};
  }
};

/// Matches one of the specified literals.
template <typename... Literals> constexpr auto literal_set(Literals...) {
...
template <typename T, template <typename> typename CaseFolding,
          typename... Strings>
constexpr auto
literal_set(const lexy::_symbol_table<T, CaseFolding, Strings...>) {
...
namespace lexy {
template <typename... Literals>
constexpr auto token_kind_of<lexy::dsl::_lset<Literals...>> =
    lexy::literal_token_kind;
} // namespace lexy
