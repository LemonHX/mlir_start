...
#include "lib/lexy/action/match.hpp"
#include "lib/lexy/dsl/base.hpp"
#include "lib/lexy/error.hpp"

namespace lexy {
struct missing_token {
  static LEXY_CONSTEVAL auto name() { return "missing token"; }
};
} // namespace lexy

//=== token_base ===//
namespace lexyd {
template <typename Tag, typename Token> struct _toke;
template <auto Kind, typename Token> struct _tokk;
...
struct _token_inherit<Tag, false> : _token_base,
                                    Tag // need to turn it into a token
{};

// ImplOrTag is either a branch base that indicates whether the token is an
// unconditional branch, or an actual token rule whose implementation will be
// used.
...
struct token_base : _token_inherit<ImplOrTag> {
  ...
  LEXY_PARSER_FUNC static bool token_parse(Context &context, Reader &reader) {
    ...
    context.on(_ev::token{}, typename Derived::token_type{}, begin,
               parser.end.position());
    reader.reset(parser.end);
...
// We cannot directly inherit from Token as that wouldn't override the
// token_type.
...
struct _toke : token_base<_toke<Tag, Token>, Token> {
  // If we're overriding the error for a char class rule, we also want to change
  // its error reporting. Otherwise, rules such as `dsl::delimited()` building
  // on char classes will generate the "wrong" error.
  //
  ...
  template <typename Reader> struct tp : lexy::token_parser_for<Token, Reader> {
    constexpr explicit tp(const Reader &reader)
        : lexy::token_parser_for<Token, Reader>(reader) {}

    template <typename Context>
    constexpr void report_error(Context &context, const Reader &reader) {
      // Report a different error.
      auto err =
          lexy::error<Reader, Tag>(reader.position(), this->end.position());
      context.on(_ev::error{}, err);
    }
  };
};
...
template <typename Tag, typename Token>
constexpr auto token_kind_of<lexy::dsl::_toke<Tag, Token>> =
    token_kind_of<Token>;
} // namespace lexy

//=== token rule ===//
namespace lexyd {
template <typename Rule> struct _token : token_base<_token<Rule>> {
  struct _production {
  ...
  template <typename Reader> struct tp {
    ...
    constexpr bool try_parse(Reader reader) {
      ...
      auto success = lexy::do_action<
          _production, lexy::match_action<void, Reader>::template result_type>(
          lexy::_mh(), lexy::no_parse_state, reader);
      end = reader.current();
      return success;
    }
...
/// Turns the arbitrary rule into a token by matching it without producing any
/// values.
template <typename Rule> constexpr auto token(Rule) {
