...
#include "lib/lexy/_detail/buffer_builder.hpp"
...
template <typename Encoding = lexy::default_encoding> struct default_prompt {
  using encoding = Encoding;
  using char_type = typename Encoding::char_type;
  static_assert(sizeof(char_type) == sizeof(char),
                "only support single-byte encodings");

  void primary_prompt() noexcept { std::fputs("> ", stdout); }

  void continuation_prompt() noexcept { std::fputs(". ", stdout); }
  ...
  auto read_line() { return read_line_callback{}; }

  struct write_message_callback {
    bool _last_was_newline = true; // If we printed nothing, it's a newline.

    void operator()(const char_type *buffer, std::size_t size) {
      std::fprintf(stdout, "%.*s", int(size),
  ...
  auto write_message() { return write_message_callback{}; }
};
} // namespace lexy_ext

namespace lexy_ext {
/// Reads input from an interactive shell.
template <typename Prompt = default_prompt<>> class shell {
public:
  ...
  bool is_open() const noexcept { return _prompt.is_open(); }
  ...
  public:
    using encoding = typename Prompt::encoding;
    using iterator =
        typename lexy::_detail::buffer_builder<char_type>::stable_iterator;
    ...
    void bump() noexcept { ++_idx; }

    auto position() const noexcept { return iterator(_shell->_buffer, _idx); }

    marker current() const noexcept { return {position()}; }
    void reset(marker m) noexcept { _idx = m._it.index(); }
  ...
  private:
    ...
    bool is_eof() const {
      if (_idx != _shell->_buffer.read_size())
        ...
        // We've reached the end of the buffer, but the user might be willing to
        // type another line.
  ...
  auto prompt_for_input() { return input(this); }
  ...
  public:
    ...
    ~writer() noexcept { LEXY_MOV(_writer).done(); }
  ...
  template <typename... Args> auto write_message(Args &&...args) {
    return writer{_prompt.write_message(LEXY_FWD(args)...)};
  }

  Prompt &get_prompt() noexcept { return _prompt; }
  const Prompt &get_prompt() const noexcept { return _prompt; }
...
private:
  ...
  bool append_next_line() {
    ...
    for (auto reader = _prompt.read_line(); true;) {
      ...
      // This grow might be unnecessary if we're just so happen to reach the
      // newline with the next character, but checking this requires reading
      // more input.
      _buffer.grow();
    }
