...
template <typename CharT, CharT... Cs> struct type_string {
...
namespace lexy::_detail {
template <std::size_t N, typename CharT> struct string_literal {
  CharT data[N];
  ...
  LEXY_CONSTEVAL string_literal(CharT c) : data{} { data[0] = c; }

  static LEXY_CONSTEVAL auto size() { return N; }
};
template <std::size_t N, typename CharT>
string_literal(const CharT (&)[N]) -> string_literal<N - 1, CharT>;
template <typename CharT> string_literal(CharT) -> string_literal<1, CharT>;
...
template <template <typename C, C... Cs> typename T, string_literal Str>
using to_type_string = decltype(_to_type_string<T, Str>(
    make_index_sequence<decltype(Str)::size()>{}));
} // namespace lexy::_detail
...
constexpr ::lexy::_detail::type_string<CharT, Cs...>
...
namespace lexy::_detail {
template <typename A, typename B> struct cat_;
...
template <typename A, typename B> using cat = typename cat_<A, B>::type;

template <template <typename CharT, CharT...> typename T, typename TypeString,
...
#define LEXY_NTTP_STRING32(Str, I)                                             \
  ::lexy::_detail::cat<LEXY_NTTP_STRING16(Str, I),                             \
                       LEXY_NTTP_STRING16(Str, I + 16)>
