...
template <typename T> struct _lazy_init_storage_trivial {
  bool _init;
  union {
    char _empty;
    T _value;
  ...
  template <typename... Args> constexpr void _construct(Args &&...args) {
    *this = _lazy_init_storage_trivial(0, LEXY_FWD(args)...);
  }
};

template <typename T> struct _lazy_init_storage_non_trivial {
  bool _init;
  union {
    char _empty;
    T _value;
  ...
  // Cannot add noexcept due to
  // https://github.com/llvm/llvm-project/issues/59854.
  ...
  LEXY_CONSTEXPR_DTOR _lazy_init_storage_non_trivial &
  operator=(_lazy_init_storage_non_trivial &&other) noexcept {
...
using _lazy_init_storage =
    std::conditional_t<_lazy_init_trivial<T>, _lazy_init_storage_trivial<T>,
                       _lazy_init_storage_non_trivial<T>>;

template <typename T> class lazy_init : _lazy_init_storage<T> {
public:
  using value_type = T;
  ...
  constexpr explicit operator bool() const noexcept { return this->_init; }
...
template <typename T> class lazy_init<T &> {
public:
  using value_type = T &;
...
template <> class lazy_init<void> {
public:
  using value_type = void;

  constexpr lazy_init() noexcept : _init(false) {}

  constexpr void emplace() { _init = true; }
  template <typename Fn, typename... Args>
  ...
  constexpr explicit operator bool() const noexcept { return _init; }
