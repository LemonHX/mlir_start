...
constexpr auto swar_length = [] {
  static_assert(sizeof(CharT) < sizeof(swar_int) &&
                sizeof(swar_int) % sizeof(CharT) == 0);
...
template <typename CharT> constexpr auto make_uchar(CharT c) {
...
template <typename CharT> using uchar_t = decltype(make_uchar(CharT()));

// Returns a swar_int filled with the specific char.
template <typename CharT> constexpr swar_int swar_fill(CharT _c) {
...
template <typename CharT> constexpr swar_int swar_fill_compl(CharT _c) {
...
template <typename CharT> struct swar_pack_result {
  swar_int value;
  swar_int mask;
  std::size_t count;
...
constexpr auto swar_pack(CharT... cs) {
  ...
  _swar_pack(result.value, -SkipFirstNChars * int(char_bit_size<char_type>),
             cs...);
  ...
  if (count <= 0) {
    ...
    result.mask =
        swar_int(swar_int(1) << count * int(char_bit_size<char_type>)) - 1;
...
template <typename CharT> constexpr bool swar_has_zero(swar_int v) {
  return swar_has_char_less<CharT, 1>(v);
}

// Returns true if v contains the specified char.
template <typename CharT, CharT C> constexpr bool swar_has_char(swar_int v) {
  if constexpr (C == 0) {
...
template <typename Derived> class swar_reader_base : _swar_base {
public:
