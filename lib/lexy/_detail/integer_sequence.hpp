...
using make_index_sequence =
    __make_integer_seq<integer_sequence, std::size_t, Size>;
...
template <class Sequence1, class Sequence2> struct concat_seq;
...
struct _make_index_sequence
    : concat_seq<typename _make_index_sequence<N / 2>::type,
                 typename _make_index_sequence<N - N / 2>::type> {};
template <> struct _make_index_sequence<0> {
  using type = index_sequence<>;
};
template <> struct _make_index_sequence<1> {
