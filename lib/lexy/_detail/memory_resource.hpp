...
public:
  ...
  static void deallocate(void *ptr, std::size_t bytes,
    ...
    // In debug mode, we fill freed memory with 0xFF to detect dangling
    // lexemes. For default, ASCII, bytes, this is just a noticable value. For
    // UTF-8, this is the EOF integer value as its an invalid code unit. For
    // UTF-16, this is the code point 0xFFFF, which is the replacement
    // character. For UTF-32, this is an out of range code point.
  ...
  friend constexpr bool operator==(default_memory_resource,
...
namespace lexy::_detail {
template <typename MemoryResource> class _memory_resource_ptr_empty {
public:
  ...
  constexpr auto operator*() const noexcept { return MemoryResource{}; }

  constexpr auto operator->() const noexcept {
    struct proxy {
      MemoryResource _resource;

      constexpr MemoryResource *operator->() noexcept { return &_resource; }
    };

    return proxy{};
  }

  constexpr MemoryResource *get() const noexcept { return nullptr; }
};

template <typename MemoryResource> class _memory_resource_ptr {
public:
  constexpr explicit _memory_resource_ptr(MemoryResource *resource) noexcept
      : _resource(resource) {
    LEXY_PRECONDITION(resource);
  }

  constexpr MemoryResource &operator*() const noexcept { return *_resource; }

  constexpr MemoryResource *operator->() const noexcept { return _resource; }

  constexpr MemoryResource *get() const noexcept { return _resource; }
...
template <typename MemoryResource,
          typename = std::enable_if_t<std::is_void_v<MemoryResource> ||
