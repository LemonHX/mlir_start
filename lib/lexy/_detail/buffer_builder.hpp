...
template <typename T> class buffer_builder {
...
public:
  buffer_builder() noexcept
      : _data(_stack_buffer), _read_size(0), _write_size(stack_buffer_size) {
    static_assert(sizeof(*this) == total_size_bytes,
  ...
  std::size_t capacity() const noexcept { return _read_size + _write_size; }

  // The read area.
  const T *read_data() const noexcept { return _data; }
  std::size_t read_size() const noexcept { return _read_size; }

  // The write area.
  T *write_data() noexcept { return _data + _read_size; }
  std::size_t write_size() const noexcept { return _write_size; }
  ...
  // Takes the first n characters of the write area and appends them to the
  // read area.
  ...
  class stable_iterator
      : public forward_iterator_base<stable_iterator, const T> {
  public:
