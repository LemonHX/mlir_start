...
#include "lib/lexy/_detail/string_view.hpp"

namespace lexy::_detail {
template <typename T>
using _detect_name_f =
    std::enable_if_t<std::is_convertible_v<decltype(T::name()), string_view>>;
template <typename T> using _detect_name_v = decltype(T::name);

template <typename T> constexpr auto _full_type_name() {
#if defined(__clang__)
#define LEXY_HAS_AUTOMATIC_TYPE_NAME 1
#define LEXY_HAS_CONSTEXPR_AUTOMATIC_TYPE_NAME 1

  constexpr auto prefix =
      string_view("auto lexy::_detail::_full_type_name() [T = ");
...
#define LEXY_HAS_CONSTEXPR_AUTOMATIC_TYPE_NAME 1
  ...
  constexpr auto prefix =
      string_view("auto __cdecl lexy::_detail::_full_type_name<");
...
template <typename T, int NsCount> constexpr string_view _type_name() {
...
template <typename T, int NsCount = 1> constexpr const char *type_name() {
...
template <typename T, int NsCount = 1> constexpr const char *const *type_id() {
