...
template <typename T> class any_holder;
...
private:
  ...
  template <typename T> friend class any_holder;
};
...
template <typename T> class any_holder : public any_base {
public:
  constexpr explicit any_holder(T &&obj) : _obj(LEXY_MOV(obj)) {}

  constexpr T &get() noexcept { return _obj; }
  constexpr const T &get() const noexcept { return _obj; }
