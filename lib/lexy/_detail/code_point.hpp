...
constexpr std::size_t encode_code_point(char32_t cp,
...
template <typename Reader> struct cp_result {
  char32_t cp;
  cp_error error;
...
constexpr cp_result<Reader> parse_code_point(Reader reader) {
  if constexpr (std::is_same_v<typename Reader::encoding,
                               lexy::ascii_encoding>) {
  ...
  } else if constexpr (std::is_same_v<typename Reader::encoding,
                                      lexy::utf8_encoding> //
                       || std::is_same_v<typename Reader::encoding,
