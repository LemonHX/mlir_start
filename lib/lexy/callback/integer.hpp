...
namespace lexy {
template <typename T> struct _int {
  using return_type = T;
...
// A callback that takes an optional sign and an integer and produces the
// signed integer.
template <typename T> constexpr auto as_integer = _int<T>{};
} // namespace lexy
