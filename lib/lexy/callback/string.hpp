...
struct _as_string {
  ...
  static constexpr String &&_case_folding(String &&str) {
    if constexpr (std::is_void_v<CaseFoldingDSL>) {
      ...
      auto original_reader =
          lexy::_range_reader<Encoding>(str.begin(), str.end());
      auto reader = typename CaseFoldingDSL::template case_folding<
          decltype(original_reader)>{original_reader};
      ...
      auto original_reader =
          lexy::_range_reader<Encoding>(original.begin(), original.end());
      auto reader = typename CaseFoldingDSL::template case_folding<
          decltype(original_reader)>{original_reader};
      while (true) {
        auto cur = reader.peek();
  ...
  constexpr String operator()(nullopt &&) const { return String(); }
  constexpr String &&operator()(String &&str) const {
    return _case_folding(LEXY_MOV(str));
  }

  template <typename Iterator>
  constexpr auto operator()(Iterator begin, Iterator end) const
      -> decltype(String(begin, end)) {
    return _case_folding(String(begin, end));
  }
  template <typename Str = String, typename Iterator>
  constexpr auto
  operator()(const typename Str::allocator_type &allocator, Iterator begin,
  ...
  struct _sink {
    ...
    template <typename CharT,
              typename = decltype(LEXY_DECLVAL(String).push_back(CharT()))>
    constexpr void operator()(CharT c) {
      _result.push_back(c);
    }

    constexpr void operator()(String &&str) { _result.append(LEXY_MOV(str)); }
    ...
    template <typename Reader> constexpr void operator()(lexeme<Reader> lex) {
      static_assert(lexy::char_type_compatible_with_reader<Reader, _char_type>,
                    "cannot convert lexeme to this string type");
    ...
    constexpr String &&finish() && { return _case_folding(LEXY_MOV(_result)); }
  };

  constexpr auto sink() const { return _sink{String()}; }
...
template <typename String,
          typename Encoding = deduce_encoding<_string_char_type<String>>>
