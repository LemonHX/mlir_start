...
constexpr auto is_brace_constructible =
    _detail::is_detected<_detect_brace_construct, T, Args...>;
...
template <typename T> struct _construct {
  ...
  constexpr T operator()(T &&t) const { return LEXY_MOV(t); }
...
template <> struct _construct<void> {
...
template <typename T> constexpr auto construct = _construct<T>{};

template <typename T, typename PtrT> struct _new {
...
/// A callback that constructs an object of type T on the heap by forwarding the
/// arguments.
