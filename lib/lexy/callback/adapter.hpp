...
struct _callback : _overloaded<Fns...> {
  ...
  constexpr explicit _callback(Fns... fns)
      : _overloaded<Fns...>(LEXY_MOV(fns)...) {}
};
...
struct _callback_with_state : _overloaded<Fns...> {
  ...
  constexpr explicit _callback_with_state(Fns... fns)
      : _overloaded<Fns...>(LEXY_MOV(fns)...) {}
...
/// Creates a callback.
template <typename... Fns> constexpr auto callback(Fns &&...fns) {
  if constexpr ((lexy::is_callback<std::decay_t<Fns>> && ...))
    return _callback<
        std::common_type_t<typename std::decay_t<Fns>::return_type...>,
...
template <typename... Fns> constexpr auto callback_with_state(Fns &&...fns) {
  if constexpr ((lexy::is_callback<std::decay_t<Fns>> && ...))
    return _callback_with_state<
        std::common_type_t<typename std::decay_t<Fns>::return_type...>,
        std::decay_t<Fns>...>(LEXY_FWD(fns)...);
  else
...
constexpr auto callback_with_state(Fns &&...fns) {
  return _callback_with_state<ReturnType, std::decay_t<Fns>...>(
      LEXY_FWD(fns)...);
}

template <typename Sink> struct _cb_from_sink {
  Sink _sink;
  ...
  constexpr auto operator()(Args &&...args) const
      -> decltype((LEXY_DECLVAL(_cb &)(LEXY_FWD(args)), ...,
...
template <typename Fn> struct _mem_fn;
template <typename MemFn, typename T> struct _mem_fn<MemFn T::*> {
  MemFn T::*_fn;
  ...
  template <typename... Args>
  constexpr auto operator()(Args &&...args) const
      -> decltype(_detail::_mem_invoker<MemFn T::*>::invoke(
          _fn, LEXY_FWD(args)...)) {
...
template <typename MemFn, typename T> constexpr auto mem_fn(MemFn T::*fn) {
