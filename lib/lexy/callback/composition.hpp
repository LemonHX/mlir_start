...
namespace lexy {
template <typename Cb, typename State, typename = void> struct _compose_state {
  const Cb &_cb;
  State &_state;
  ...
  constexpr auto
  operator()(Args &&...args) const -> decltype(_cb(LEXY_FWD(args)...)) {
    return _cb(LEXY_FWD(args)...);
  }
};
template <typename Cb, typename State>
struct _compose_state<Cb, State,
                      std::enable_if_t<lexy::is_callback_state<Cb, State>>> {
  const Cb &_cb;
  State &_state;
  ...
  template <typename... Args>
  constexpr auto
  operator()(Args &&...args) const -> decltype(_cb[_state](LEXY_FWD(args)...)) {
    return _cb[_state](LEXY_FWD(args)...);
  }
};

template <typename First, typename Second> struct _compose_cb {
  ...
  template <typename State, typename = std::enable_if_t<
                                lexy::is_callback_state<First, State> //
  ...
  constexpr auto operator()(Args &&...args) const
      -> LEXY_DECAY_DECLTYPE(_first(LEXY_FWD(args)...),
                             LEXY_DECLVAL(return_type)) {
    return _second(_first(LEXY_FWD(args)...));
  }
};

template <typename Sink, typename Callback> struct _compose_s {
  ...
  template <typename... Args>
  constexpr auto
  sink(Args &&...args) const -> decltype(_sink.sink(LEXY_FWD(args)...)) {
    return _sink.sink(LEXY_FWD(args)...);
  }

  template <typename State, typename = std::enable_if_t<
                                lexy::is_callback_state<Callback, State>>>
  constexpr auto operator[](State &state) const {
  ...
  template <typename... Args>
  constexpr auto
  operator()(Args &&...args) const -> decltype(_callback(LEXY_FWD(args)...)) {
