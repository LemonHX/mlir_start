...
#include "lib/lexy/_detail/tuple.hpp"
...
expand_bound_arg(const BoundArg &bound, State &state,
...
template <std::size_t Idx, typename Fn, typename... BoundArgs, typename State,
          typename... ActualArgs, std::size_t... ActualIdx,
          typename... ProducedArgs>
constexpr decltype(auto)
_invoke_bound(Fn &&fn, const _detail::tuple<BoundArgs...> &bound_args,
              State &state, const _detail::tuple<ActualArgs...> &actual_args,
...
template <typename Fn, typename... BoundArgs, std::size_t... Idx,
          typename State,
          typename... Args>
constexpr decltype(auto)
invoke_bound(Fn &&fn, //
  ...
  if constexpr ((_detail::is_decayed_same<BoundArgs,
                                          _detail::all_values_placeholder> ||
                 ...)) {
    // If we're having the placeholder we need to recursively expand every
    // argument.
    return _invoke_bound<0>(LEXY_FWD(fn), bound_args, state, actual_args,
                            actual_args.index_sequence());
  } else {
    // If we're not having the all_values_placeholder, every placeholder
    // expands to a single argument. We can thus expand it easily by mapping
    // each value of bound args to the actual argument.
    return LEXY_FWD(fn)(expand_bound_arg(bound_args.template get<Idx>(), state,
                                         actual_args)...);
  }
}
} // namespace lexy::_detail
...
struct _nth_value : _detail::placeholder_base // fallback + map
{
  ...
  LEXY_FORCE_INLINE constexpr decltype(auto)
  operator()(State &, const _detail::tuple<Args...> &args) const {
...
struct _nth_value<N, void, Fn> : _detail::placeholder_base // map only
{
  ...
  template <typename Arg> constexpr auto operator||(Arg &&fallback) const {
    return _nth_value<N, std::decay_t<Arg>, Fn>{{}, LEXY_FWD(fallback), _fn};
  }
  template <typename Arg> constexpr auto or_(Arg &&fallback) const {
    return *this || LEXY_FWD(fallback);
  }

  constexpr auto or_default() const { return *this || _default{}; }
};
...
template <std::size_t N> constexpr auto nth_value = _nth_value<N, void, void>{};
...
template <typename Fn> struct _parse_state : _detail::placeholder_base {
  LEXY_EMPTY_MEMBER Fn _fn;
  ...
  constexpr decltype(auto) operator()(State &state,
    ...
    static_assert(
        !std::is_same_v<State, _detail::no_bind_state>,
        "lexy::parse_state requires that a state is passed to lexy::parse()");
    return _detail::invoke(_fn, state);
  }
};
template <> struct _parse_state<void> : _detail::placeholder_base {
  template <typename State, typename... Args>
  constexpr decltype(auto) operator()(State &state,
                                      const _detail::tuple<Args...> &) const {
    static_assert(
...
//=== bind ===//
namespace lexy {
template <typename Callback, typename... BoundArgs> struct _bound_cb {
  ...
  constexpr return_type operator()(Args &&...args) const {
    ...
    return _detail::invoke_bound(_callback, _bound_args,
                                 _bound_args.index_sequence(), state,
                                 LEXY_FWD(args)...);
  }
};
...
namespace lexy {
template <typename Sink> struct _sink_wrapper {
  const Sink &_sink;
...
template <typename Sink, typename... BoundArgs> struct _bound_sink {
  LEXY_EMPTY_MEMBER Sink _sink;
  LEXY_EMPTY_MEMBER _detail::tuple<BoundArgs...> _bound;

  template <typename... Args>
  constexpr auto
  operator()(Args... args) const -> decltype(_sink(LEXY_FWD(args)...)) {
    return _sink(LEXY_FWD(args)...);
  }

  template <bool Dummy = true,
            typename = std::enable_if_t<(!_detail::is_placeholder<BoundArgs> &&
                                         ... && Dummy)>>
  constexpr auto sink() const {
    auto state = _detail::no_bind_state{};
    return _detail::invoke_bound(_sink_wrapper<Sink>{_sink}, _bound,
                                 _bound.index_sequence(), state);
  }
...
/// The result has a `.sink()` function that accepts the state (i.e. the parse
/// state), but no additional values.
...
constexpr auto bind_sink(Sink &&sink, BoundArgs &&...args) {
  static_assert(
      (!_detail::is_decayed_same<BoundArgs, _detail::all_values_placeholder> &&
       ...),
      "lexy::values as a placeholder for bind_sink() doesn't make sense - "
      "there won't be any values");
