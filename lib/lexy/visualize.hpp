...
#include "lib/lexy/input/range_input.hpp"
...
enum visualization_flags {
  ...
  /// Visualization can use Unicode symbols e.g. for newline/space instead of
  /// the code point.
...
struct visualization_options {
  ...
  friend constexpr visualization_options
  operator|(visualization_options lhs, visualization_flags rhs) noexcept {
...
constexpr OutIt write_special_char(OutIt out, visualization_options opts,
                                   Fn inner) {
  out = _detail::write_color<_detail::color::cyan, _detail::color::faint>(out,
                                                                          opts);
  if (opts.is_set(visualize_use_unicode))
...
OutputIt visualize_to(OutputIt out, lexy::code_point cp,
...
OutputIt visualize_to(OutputIt out, lexy::lexeme<Reader> lexeme,
  ...
  // We need to ensure that we're not printing more "code points" than
  // `opts.max_lexeme_width`, or unlimited if `opts.max_lexeme_width == 0`. The
  // trick is to count and check for `count == opts.max_lexeme_width` after
  // increment.

  [[maybe_unused]] auto write_escaped_byte = [opts](OutputIt out,
                                                    unsigned char byte) {
  ...
  if constexpr (lexy::is_unicode_encoding<encoding>) {
    ...
    lexy::range_input<encoding, typename Reader::iterator> input(lexeme.begin(),
                                                                 lexeme.end());
    ...
    for (char c : lexeme) {
      ...
      if (lexy::_detail::is_ascii(c))
        out =
            visualize_to(out, lexy::code_point(static_cast<char32_t>(c)), opts);
      else
    ...
    lexy::range_input<typename encoding::char_encoding,
                      typename Reader::iterator>
        input(lexeme.begin(), lexeme.end());
    return visualize_to(
        out, lexy::lexeme_for<decltype(input)>(input.begin(), input.end()));
  } else {
...
template <typename OutputIt, typename Tree,
          typename = decltype(LEXY_DECLVAL(Tree &).traverse())>
OutputIt visualize_to(OutputIt out, const Tree &tree,
                      visualization_options opts = {}) {
  struct label_t {
  ...
  auto label = opts.is_set(visualize_use_unicode)
                   ? label_t{u8"   ", u8"│  ", u8"└──", u8"├──"}
                   : label_t{u8"  ", u8"  ", u8"- ", u8"- "};

  // True if the node currently opened at the depth is the last child of its
  ...
  auto write_prefix = [opts, label, &is_last_child](OutputIt out,
...
std::size_t visualization_display_width(const T &obj,
  ...
  struct iterator {
    ...
    iterator &operator=(char c) {
      ...
      // so we just need to count all code units that are not continuation
      // bytes.
  ...
  // Disable usage of color, as they introduce additional characters that must
  // nobe counted.
