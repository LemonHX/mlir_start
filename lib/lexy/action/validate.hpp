...
#include "lib/lexy/_detail/any_ref.hpp"
#include "lib/lexy/_detail/lazy_init.hpp"
#include "lib/lexy/action/base.hpp"
#include "lib/lexy/callback/base.hpp"
#include "lib/lexy/callback/container.hpp"
#include "lib/lexy/callback/noop.hpp"
...
constexpr auto _get_error_sink(const Callback &callback) {
  if constexpr (std::is_same_v<Callback, lexy::_noop>) {
    ...
    static_assert(std::is_void_v<typename Callback::return_type>,
                  "need to use `lexy::collect()` to create an error callback "
                  "that can handle multiple errors");
...
public:
  ...
  constexpr const auto &errors() const & noexcept { return _error; }
  constexpr auto &&errors() && noexcept { return LEXY_MOV(_error); }
...
private:
  ...
  template <typename Reader> friend class _vh;
};
} // namespace lexy

namespace lexy {
template <typename Reader> struct _validate_callbacks {
  _detail::any_ref sink;
  _detail::any_cref input;

  void (*generic)(_detail::any_ref sink, production_info info,
                  _detail::any_cref input, typename Reader::iterator begin,
                  const error<Reader, void> &error);
  void (*literal)(_detail::any_ref sink, production_info info,
                  _detail::any_cref input, typename Reader::iterator begin,
                  const error<Reader, expected_literal> &error);
  void (*keyword)(_detail::any_ref sink, production_info info,
                  _detail::any_cref input, typename Reader::iterator begin,
                  const error<Reader, expected_keyword> &error);
  void (*char_class)(_detail::any_ref sink, production_info info,
                     _detail::any_cref input, typename Reader::iterator begin,
  ...
  constexpr _validate_callbacks(const _detail::any_holder<const Input *> &input,
      ...
      : sink(&sink), input(&input),
        generic([](_detail::any_ref sink, production_info info,
                   _detail::any_cref input, typename Reader::iterator begin,
                   const error<Reader, void> &error) {
          lexy::error_context err_ctx(
              info, *input->template get<const Input *>(), begin);
          sink->template get<Sink>()(err_ctx, LEXY_FWD(error));
        }),
        literal([](_detail::any_ref sink, production_info info,
                   _detail::any_cref input, typename Reader::iterator begin,
                   const error<Reader, expected_literal> &error) {
          lexy::error_context err_ctx(
              info, *input->template get<const Input *>(), begin);
        ...
        keyword([](_detail::any_ref sink, production_info info,
                   _detail::any_cref input, typename Reader::iterator begin,
                   const error<Reader, expected_keyword> &error) {
          lexy::error_context err_ctx(
              info, *input->template get<const Input *>(), begin);
        ...
        char_class([](_detail::any_ref sink, production_info info,
                      _detail::any_cref input, typename Reader::iterator begin,
                      const error<Reader, expected_char_class> &error) {
          lexy::error_context err_ctx(
              info, *input->template get<const Input *>(), begin);
...
template <typename Reader> class _vh {
public:
  template <typename Input, typename Sink>
  ...
  public:
    ...
    constexpr void on(_vh &handler, parse_events::production_start,
                      iterator pos) {
      _begin = pos;
    ...
    constexpr void on(_vh &handler, parse_events::error,
      ...
      handler._cb.generic(handler._cb.sink, get_info(), handler._cb.input,
                          _begin, error);
    }
    template <typename R>
    constexpr void on(_vh &handler, parse_events::error,
    ...
    constexpr void on(_vh &handler, parse_events::error,
      ...
      handler._cb.literal(handler._cb.sink, get_info(), handler._cb.input,
                          _begin, error);
    }
    template <typename R>
    constexpr void on(_vh &handler, parse_events::error,
                      const error<R, expected_keyword> &error) {
      handler._cb.keyword(handler._cb.sink, get_info(), handler._cb.input,
                          _begin, error);
    }
    template <typename R>
    constexpr void on(_vh &handler, parse_events::error,
                      const error<R, expected_char_class> &error) {
      handler._cb.char_class(handler._cb.sink, get_info(), handler._cb.input,
                             _begin, error);
    }
    ...
    constexpr iterator production_begin() const { return _begin; }
  ...
  constexpr auto get_result(bool rule_parse_result) && {
    ...
    return Result(rule_parse_result,
                  LEXY_MOV(_cb.sink->template get<sink_t>()).finish());
  }
...
struct validate_action {
  ...
  template <typename> using result_type = validate_result<ErrorCallback>;

  constexpr explicit validate_action(const ErrorCallback &callback)
      : _callback(&callback) {}
  ...
  constexpr auto operator()(Production, const Input &input) const {
    ...
    return lexy::do_action<Production, result_type>(handler(input_holder, sink),
                                                    _state, reader);
  }
};
...
constexpr auto validate(const Input &input, const ErrorCallback &callback)
  ...
  return validate_action<void, Input, ErrorCallback>(callback)(Production{},
                                                               input);
}
...
constexpr auto
validate(const Input &input, State &state,
         const ErrorCallback &callback) -> validate_result<ErrorCallback> {
  return validate_action<State, Input, ErrorCallback>(state, callback)(
      Production{}, input);
}
template <typename Production, typename Input, typename State,
          typename ErrorCallback>
constexpr auto
validate(const Input &input, const State &state,
         const ErrorCallback &callback) -> validate_result<ErrorCallback> {
  return validate_action<const State, Input, ErrorCallback>(state, callback)(
      Production{}, input);
}
} // namespace lexy
