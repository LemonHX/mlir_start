...
struct parse_context_var_base {
  ...
  template <typename Context> constexpr void link(Context &context) {
    auto cb = context.control_block;
    next = cb->vars;
    cb->vars = this;
  }

  template <typename Context> constexpr void unlink(Context &context) {
    auto cb = context.control_block;
    cb->vars = next;
  }
};
...
struct parse_context_control_block {
  ...
  constexpr parse_context_control_block(Handler &&handler, State *state,
      ...
      : parse_handler(LEXY_MOV(handler)), parse_state(state), //
        vars(nullptr),                                        //
        cur_depth(0), max_depth(static_cast<int>(max_depth)),
        enable_whitespace_skipping(true) {}

  template <typename OtherHandler>
  constexpr parse_context_control_block(
      Handler &&handler, parse_context_control_block<OtherHandler, State> *cb)
  ...
  template <typename OtherHandler>
  constexpr void
  copy_vars_from(parse_context_control_block<OtherHandler, State> *cb) {
    vars = cb->vars;
...
// If a production doesn't define whitespace, we don't need to pass it and can
// shorten the template name.
template <typename Production>
using _whitespace_production_of =
    std::conditional_t<_production_defines_whitespace<Production>, Production,
                       void>;
...
struct _pc {
  ...
  constexpr explicit _pc(
      _detail::parse_context_control_block<Handler, State> *cb)
  ...
  constexpr auto sub_context(ChildProduction) {
    ...
    using new_whitespace_production = std::conditional_t<
        is_token_production<ChildProduction> //
            || _production_defines_whitespace<ChildProduction>,
        _whitespace_production_of<ChildProduction>, WhitespaceProduction>;
    return _pc<Handler, State, ChildProduction, new_whitespace_production>(
        control_block);
  }
...
struct final_parser {
  ...
  LEXY_PARSER_FUNC static bool parse(Context &context, Reader &,
                                     Args &&...args) {
...
template <typename NextParser> struct context_finish_parser {
  template <typename Context, typename Reader, typename SubContext,
            typename... Args>
  LEXY_PARSER_FUNC static bool parse(Context &context, Reader &reader,
                                     SubContext &sub_context, Args &&...args) {
    // Might need to skip whitespace, according to the original context.
    using continuation = std::conditional_t<
        lexy::is_token_production<typename SubContext::production>,
...
template <typename Handler, typename State, typename Production,
          typename Reader>
constexpr auto _do_action(_pc<Handler, State, Production> &context,
                          Reader &reader) {
  ...
  using parser = lexy::whitespace_parser<
      ...
      lexy::parser_for<lexy::production_rule<Production>,
                       _detail::final_parser>>;
...
template <typename Production, template <typename> typename Result,
          typename Handler, typename State, typename Reader>
constexpr auto do_action(Handler &&handler, State *state, Reader &reader) {
  ...
  _detail::parse_context_control_block control_block(
      LEXY_MOV(handler), state, max_recursion_depth<Production>());
  ...
  if constexpr (std::is_void_v<value_type>)
    return LEXY_MOV(control_block.parse_handler)
        .template get_result<Result<void>>(rule_result);
...
struct void_value_callback {
  ...
  template <typename State> constexpr explicit void_value_callback(State *) {}

  using return_type = void;

  constexpr auto sink() const { return lexy::noop.sink(); }
