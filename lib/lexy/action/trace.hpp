...
#include "lib/lexy/input_location.hpp"
#include "lib/lexy/token.hpp"
#include "lib/lexy/visualize.hpp"
...
template <typename OutputIt, typename TokenKind> class trace_writer {
public:
  ...
  void write_token(const Location &loc, lexy::token_kind<TokenKind> kind,
    ...
    if (_cur_depth > _opts.max_tree_depth ||
        (kind.ignore_if_empty() && lexeme.empty()))
      return;
  ...
  void write_backtrack(const Location &loc, lexy::lexeme<Reader> lexeme) {
    ...
    _out = _detail::write_color<_detail::color::yellow, _detail::color::bold>(
        _out, _opts);
  ...
  void write_error(const Location &loc, const lexy::error<Reader, Tag> &error) {
    ...
    _out = _detail::write_color<_detail::color::red, _detail::color::bold>(
        _out, _opts);
    ...
    if constexpr (std::is_same_v<Tag, lexy::expected_literal>) {
      auto string = _detail::make_literal_lexeme<typename Reader::encoding>(
  ...
  template <typename Location> void write_recovery_start(const Location &loc) {
    if (_cur_depth <= _opts.max_tree_depth) {
      write_prefix(loc, prefix::event);
  ...
  void write_debug(const Location &loc, const char *str) {
    ...
    _out = _detail::write_color<_detail::color::blue, _detail::color::bold>(
        _out, _opts);
  ...
  template <typename Location> void write_finish(const Location &loc) {
    if (_cur_depth <= _opts.max_tree_depth)
      write_prefix(loc, prefix::finish);
    --_cur_depth;
  }
  template <typename Location> void write_cancel(const Location &loc) {
    if (_cur_depth <= _opts.max_tree_depth)
...
private:
  ...
  void write_prefix(const Location &loc, prefix p) {
    ...
    _out =
        _detail::write_format(_out, "%2u:%3u", loc.line_nr(), loc.column_nr());
    _out = _detail::write_str(_out, ": ");
    _out = _detail::write_color<_detail::color::reset>(_out, _opts);
...
class _th {
public:
  explicit _th(OutputIt out, const Input &input,
               visualization_options opts = {}) noexcept
      : _writer(out, opts), _input(&input), _anchor(input) {
    LEXY_PRECONDITION(opts.max_tree_depth <=
                      visualization_options::max_tree_depth_limit);
  }
  ...
  public:
    ...
    void on(_th &handler, parse_events::operation_chain_op, Operation,
            iterator pos) {
      auto loc = handler.get_location(pos);
      handler._writer.write_operation(loc, lexy::production_name<Operation>());
    }
    void on(_th &handler, parse_events::operation_chain_finish, int,
            iterator pos) {
      auto loc = handler.get_location(pos);
      handler._writer.write_finish(loc);
    }
    ...
    void on(_th &handler, parse_events::token, TK kind, iterator begin,
            iterator end) {
      auto loc = handler.get_location(begin);
      handler._writer.write_token(loc, token_kind<TokenKind>(kind),
                                  lexeme_for<Input>(begin, end));
    }
    void on(_th &handler, parse_events::backtracked, iterator begin,
            iterator end) {
      auto loc = handler.get_location(begin);
  ...
  template <typename> constexpr OutputIt get_result(bool) && {
    return LEXY_MOV(_writer).finish();
  }

private:
  input_location<Input>
  get_location(typename lexy::input_reader<Input>::iterator pos) {
...
template <typename State, typename Input, typename OutputIt,
          typename TokenKind = void>
struct trace_action {
  OutputIt _out;
  visualization_options _opts;
  ...
  template <typename> using result_type = OutputIt;

  constexpr explicit trace_action(OutputIt out, visualization_options opts = {})
      : _out(out), _opts(opts) {}
  template <typename U = State>
  constexpr explicit trace_action(U &state, OutputIt out,
  ...
  constexpr auto operator()(Production, const Input &input) const {
    ...
    return lexy::do_action<Production, result_type>(handler(_out, input, _opts),
                                                    _state, reader);
  }
};

template <typename Production, typename TokenKind = void, typename OutputIt,
          typename Input>
OutputIt trace_to(OutputIt out, const Input &input,
                  visualization_options opts = {}) {
  return trace_action<void, Input, OutputIt, TokenKind>(out, opts)(Production{},
                                                                   input);
}
template <typename Production, typename TokenKind = void, typename OutputIt,
          typename Input, typename State>
OutputIt trace_to(OutputIt out, const Input &input, State &state,
                  visualization_options opts = {}) {
  return trace_action<State, Input, OutputIt, TokenKind>(state, out, opts)(
      Production{}, input);
}
template <typename Production, typename TokenKind = void, typename OutputIt,
          typename Input, typename State>
OutputIt trace_to(OutputIt out, const Input &input, const State &state,
                  visualization_options opts = {}) {
  return trace_action<const State, Input, OutputIt, TokenKind>(
      state, out, opts)(Production{}, input);
}

template <typename Production, typename TokenKind = void, typename Input>
void trace(std::FILE *file, const Input &input,
           visualization_options opts = {}) {
  trace_to<Production, TokenKind>(cfile_output_iterator{file}, input, opts);
}
template <typename Production, typename TokenKind = void, typename Input,
          typename State>
void trace(std::FILE *file, const Input &input, State &state,
           visualization_options opts = {}) {
  trace_to<Production, TokenKind>(cfile_output_iterator{file}, input, state,
                                  opts);
}
template <typename Production, typename TokenKind = void, typename Input,
          typename State>
void trace(std::FILE *file, const Input &input, const State &state,
           visualization_options opts = {}) {
  trace_to<Production, TokenKind>(cfile_output_iterator{file}, input, state,
                                  opts);
}
