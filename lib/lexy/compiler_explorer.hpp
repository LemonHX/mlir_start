...
#include "lib/lexy/input/buffer.hpp"
#include <cstdio>
...
lexy::buffer<lexy::utf8_encoding> compiler_explorer_input() {
  ...
  while (true) {
    ...
    // This grow might be unnecessary if we're just so happen to reach EOF
    // with the next input, but checking this requires reading more input.
    builder.grow();
  }

  return lexy::buffer<lexy::utf8_encoding>(builder.read_data(),
                                           builder.read_size());
}
...
using compiler_explorer_error_context =
