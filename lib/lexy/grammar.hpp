...
#include "lib/lexy/_detail/detect.hpp"
#include "lib/lexy/_detail/type_name.hpp"
...
constexpr bool is_unconditional_branch_rule =
    std::is_base_of_v<dsl::unconditional_branch_base, T>;
...
#define LEXY_REQUIRE_BRANCH_RULE(Rule, Name)                                   \
  static_assert(lexy::_require_branch_rule<Rule>,                              \
                Name " requires a branch condition."                           \
                     " You may need to use `>>` to specify the condition "     \
                     "that is used for dispatch."                              \
                     " See https://lexy.foonathan.net/learn/branching/ for "   \
                     "more information.")
...
constexpr bool is_production =
    _detail::is_detected<production_rule, Production>;
...
///
/// When generating a parse tree, it will also merge tokens of the same kind
/// into the same node.
struct token_production {};

template <typename Production>
constexpr bool is_token_production =
    std::is_base_of_v<token_production, Production>;

/// Base class to indicate that this production is transparent for the parse
/// tree generation. It will not create a node in the tree, all children will be
/// added to the its parent. If parse tree generation is not used, it has no
/// effect.
struct transparent_production {};

template <typename Production>
constexpr bool is_transparent_production =
    std::is_base_of_v<transparent_production, Production>;

template <typename Production> LEXY_CONSTEVAL const char *production_name() {
...
LEXY_CONSTEVAL std::size_t max_recursion_depth() {
  if constexpr (_detail::is_detected<_detect_max_recursion_depth,
                                     EntryProduction>)
...
template <typename T>
using _enable_production_or_operation =
    std::enable_if_t<is_production<T> || is_operation<T>>;
...
template <typename Production, typename WhitespaceProduction>
using production_whitespace =
    decltype(_production_whitespace<Production, WhitespaceProduction>());
} // namespace lexy
...
template <> inline constexpr auto _is_convertible<void> = true;

template <typename ParseState, typename Production>
using _detect_value_of =
    // We're testing a non-const ParseState on purpose, to handle cases where a
    // user forgot to const qualify value_of() (it causes a hard error instead
    // of going to ::value).
    typename decltype(LEXY_DECLVAL(ParseState &)
                          .value_of(Production{}))::return_type;

template <typename Production>
using _detect_value = decltype(Production::value);

template <typename Production, typename Sink> struct _sfinae_sink {
  Sink _sink;

  using return_type = typename Sink::return_type;

  LEXY_FORCE_INLINE constexpr _sfinae_sink(Production, Sink &&sink)
      : _sink(LEXY_MOV(sink)) {}

  template <typename... Args>
  LEXY_FORCE_INLINE constexpr void operator()(Args &&...args) {
    if constexpr (!is_sink_callback_for<Sink, Args &&...>)
      // We're attempting to call a sink of Production with the given arguments,
      // but no such overload exists.
...
class production_value_callback {
  static constexpr auto _get_value([[maybe_unused]] ParseState *state) {
    if constexpr (lexy::_detail::is_detected<_detect_value_of, ParseState,
                                             Production>)
  ...
  static auto _return_type_callback() {
    if constexpr (lexy::is_callback<_type>)
      return _get_value(nullptr);
                                     ...
                                     std::add_lvalue_reference_t<ParseState>>)
...
public:
  constexpr explicit production_value_callback(ParseState *state)
      : _state(state) {}

  template <typename State = ParseState,
            typename = std::enable_if_t<std::is_void_v<State>>>
  ...
  constexpr auto sink() const {
    if constexpr (lexy::is_sink<_type,
                                std::add_lvalue_reference_t<ParseState>>) {
      return _sfinae_sink(Production{}, _get_value(_state).sink(*_state));
    } else {
      // We're attempting to obtain a sink for Production, but none was
      // provided. As Production uses a list rule, it needs a sink.
  ...
  constexpr return_type operator()(Args &&...args) const {
    if constexpr (lexy::is_callback_with_state_for<_type, ParseState,
                                                   Args &&...> &&
      ...
      return _get_value(_state)(LEXY_FWD(args)...);
                          ...
                          || lexy::is_sink<_type, std::add_lvalue_reference_t<
                                                      ParseState>>) //
      ...
      // Assume it came from the list sink and return the value without invoking
      // a callback.
      return (LEXY_FWD(args), ...);
    } else {
      // We're attempting to call the callback of Production with the given
      // arguments, but no such overload exists.
      static_assert(_detail::error<Production, Args...>,
