...
#include "lib/lexy/_detail/assert.hpp"
#include "lib/lexy/_detail/config.hpp"
#include "lib/lexy/_detail/iterator.hpp"
#include "lib/lexy/_detail/memory_resource.hpp"
#include "lib/lexy/grammar.hpp"
#include "lib/lexy/token.hpp"

namespace lexy {
template <typename Node> struct parse_tree_input_traits;
}

//=== internal: pt_node ===//
namespace lexy::_detail {
template <typename Reader> class pt_node;
template <typename Reader> struct pt_node_token;
template <typename Reader> struct pt_node_production;
...
public:
  ...
  unsigned type() const noexcept { return _value & 0b1; }

  auto as_token() noexcept {
    return type() == type_token ? static_cast<pt_node_token<Reader> *>(this)
                                : nullptr;
  }
  auto as_production() noexcept {
    return type() == type_production
               ? static_cast<pt_node_production<Reader> *>(this)
               : nullptr;
  }
  ...
  unsigned next_role() const noexcept { return (_value & 0b10) >> 1; }
...
private:
  static std::uintptr_t _make_packed_ptr(pt_node<Reader> *ptr, unsigned type,
                                         unsigned role) {
  ...
  // Stores a bit that remembers whether the next node is a sibling or parent
  // (the role). Stores a bit that remembers whether *this* node is a token or
  // production.
  std::uintptr_t _value;
};

template <typename Reader> struct pt_node_token : pt_node<Reader> {
  ...
  using _end_t
      // If we can optimize it, we store the size as a uint32_t, otherwise the
      // iterator.
      = std::conditional_t<_optimize_end, std::uint_least32_t,
  ...
  explicit pt_node_token(std::uint_least16_t kind,
                         typename Reader::iterator begin,
...
template <typename Reader> struct pt_node_production : pt_node<Reader> {
  ...
  explicit pt_node_production(production_info info) noexcept
      : pt_node<Reader>(pt_node<Reader>::type_production), id(info.id),
        child_count(0), token_production(info.is_token),
        first_child_adjacent(true) {
...
template <typename MemoryResource> class pt_buffer {
  ...
  struct block {
    ...
    unsigned char *end() noexcept { return &memory[block_size]; }
  };

public:
  //=== constructors/destructors/assignment ===//
  explicit constexpr pt_buffer(MemoryResource *resource) noexcept
      : _resource(resource), _head(nullptr), _cur_block(nullptr),
        _cur_pos(nullptr) {}

  pt_buffer(pt_buffer &&other) noexcept
      : _resource(other._resource), _head(other._head),
        _cur_block(other._cur_block), _cur_pos(other._cur_pos) {
  ...
  // (If done in the constructor, it would require a move that does allocation
  // which we don't want). If called after being initialized, destroys all nodes
  ...
  template <typename T, typename... Args> T *allocate(Args &&...args) {
    ...
    constexpr auto size =
        sizeof(T); // NOLINT: It's fine, we want to allocate for a pointer.
    LEXY_PRECONDITION(_cur_block); // Forgot to call .init().
    LEXY_PRECONDITION(remaining_capacity() >=
                      size); // Forgot to call .reserve().
  ...
  void unwind(void *marker) noexcept {
    ...
    // However, on all implementations I care about, std::less just does <
    // anyway.
    if (_cur_block->memory <= pos && pos < _cur_block->end())
      ...
      // unwind() is only used to backtrack a production, which happens after a
      // couple of tokens only; the memory waste is directly proportional to the
      // lookahead length.
...
//=== parse_tree ===//
namespace lexy {
template <typename Reader, typename TokenKind> class _pt_node_kind;
template <typename Reader, typename TokenKind> class _pt_node;
...
public:
  ...
  constexpr parse_tree()
      : parse_tree(_detail::get_memory_resource<MemoryResource>()) {}
...
using parse_tree_for =
    lexy::parse_tree<lexy::input_reader<Input>, TokenKind, MemoryResource>;
...
public:
  ...
  private:
    ...
    explicit marker(void *unwind_pos, std::size_t cur_depth,
        ...
        : unwind_pos(unwind_pos), prod(prod), child_count(0),
          first_child(nullptr), last_child(nullptr), cur_depth(cur_depth),
          local_max_depth(cur_depth) {}
    ...
    void insert_children_into(_detail::pt_node_production<Reader> *parent) {
      ...
      if (first_child == parent + 1) {
        ...
        // This case happens either if the first child is in a new block or if
        // we're dealing with a container node. In either case, we've already
        // reserved memory for a pointer.
      ...
      constexpr auto mask =
          ((
               std::size_t(1)
               << _detail::pt_node_production<Reader>::child_count_bits) -
           1);
  ...
  //=== root node ===//
  explicit builder(parse_tree &&tree, production_info production)
      : _result(LEXY_MOV(tree)) {
    ...
    _result._root =
        _result._buffer.template allocate<_detail::pt_node_production<Reader>>(
            production);
    _result._size = 1;
    _result._depth = 0;
  ...
  explicit builder(production_info production)
      : builder(parse_tree(), production) {}

  [[deprecated("Pass the remaining input, or `input.end()` if there is "
               "none.")]] parse_tree &&
  finish() && {
  ...
  parse_tree &&finish(lexy::lexeme<Reader> remaining_input) && {
    ...
    auto node =
        _result._buffer.template allocate<_detail::pt_node_token<Reader>>(
            lexy::eof_token_kind, remaining_input.begin(),
            remaining_input.end());
  ...
  auto start_production(production_info production) {
    ...
    // Allocate a node for the production and append it to the current child
    // list. We reserve enough memory to allow for a trailing pointer. This is
    // only necessary if the first child is in a new block, in which case we
    // won't overwrite it.
    ...
    // Subsequent insertions are to the new node, so update marker and return
    // old one.
  ...
  marker start_container() {
    ...
    if (_cur.prod && _cur.child_count == 0) {
      ...
      //
      // If our parent is another container, its start_container() function took
      // care of it. If our parent already has a child, the first_child pointer
      // is taken care of.
  ...
  void set_container_production(production_info production) {
    ...
    // Create a new container that will contain the production as its only
    // child. As such, it logically starts at the same position and depth as the
    // current container.
    ...
    // The local_max_depth of the new container is determined by the old maximum
    // depth + 1.
  ...
  void token(token_kind<TokenKind> _kind, typename Reader::iterator begin,
    ...
    if (kind == lexy::error_token_kind && _cur.last_child &&
  ...
  //=== accessors ===//
  std::size_t current_child_count() const noexcept { return _cur.child_count; }
...
template <typename Reader, typename TokenKind> class _pt_node_kind {
public:
  bool is_token() const noexcept { return _ptr->as_token() != nullptr; }
  bool is_production() const noexcept {
    return _ptr->as_production() != nullptr;
  }

  bool is_root() const noexcept {
    // Root node has a next node (the remaining input node) which has no next
    // node. We assume that _ptr is never the remaining input node, so we know
    // that we have a next node.
...
template <typename Reader, typename TokenKind> class _pt_node {
public:
  void *address() const noexcept { return _ptr; }

  auto kind() const noexcept { return _pt_node_kind<Reader, TokenKind>(_ptr); }
  ...
  public:
    class iterator : public _detail::forward_iterator_base<iterator, _pt_node,
                                                           _pt_node, void> {
    public:
      iterator() noexcept : _cur(nullptr) {}

      auto deref() const noexcept { return _pt_node(_cur); }

      void increment() noexcept { _cur = _cur->next_node(); }

      bool equal(iterator rhs) const noexcept { return _cur == rhs._cur; }
  ...
  auto children() const noexcept { return children_range(_ptr); }
  ...
  private:
    explicit sibling_range(_detail::pt_node<Reader> *node) noexcept
        : _node(node) {}
  ...
  auto siblings() const noexcept { return sibling_range(_ptr); }
...
public:
  ...
  class iterator : public _detail::forward_iterator_base<iterator, _value_type,
                                                         _value_type, void> {
  public:
    iterator() noexcept = default;

    _value_type deref() const noexcept { return {_ev, node(_cur)}; }
...
struct parse_tree_input_traits<_pt_node<Reader, TokenKind>> {
  ...
  static bool is_null(_node cur) noexcept { return cur._ptr == nullptr; }

  static _node null() noexcept { return _node(nullptr); }
