...
template <typename Input> struct input_location_anchor {
...
public:
  template <typename Reader> constexpr bool try_match_newline(Reader &reader) {
    static_assert(lexy::is_char_encoding<typename Reader::encoding>);
    return lexy::try_match_token(lexy::dsl::newline, reader);
  }

  template <typename Reader> constexpr void match_column(Reader &reader) {
    static_assert(lexy::is_char_encoding<typename Reader::encoding>);
    reader.bump();
  }
};
...
template <std::size_t LineWidth = 16> class byte_location_counting {
public:
  template <typename Reader> constexpr bool try_match_newline(Reader &reader) {
...
template <typename Input> auto _compute_default_location_counting() {
...
using _default_location_counting =
    decltype(_compute_default_location_counting<Input>());
} // namespace lexy
...
public:
  constexpr explicit input_location(const Input &input)
      : _line_begin(input.reader().current()),
        _column_begin(_line_begin.position()), _line_nr(1), _column_nr(1) {}
  ...
  constexpr unsigned line_nr() const { return _line_nr; }
  constexpr unsigned column_nr() const { return _column_nr; }

  /// The corresponding position, rounded down to the previous column start.
  constexpr iterator position() const { return _column_begin; }

  friend constexpr bool operator==(const input_location &lhs,
                                   const input_location &rhs) {
...
private:
  constexpr input_location(marker line_begin, unsigned line_nr,
                           iterator column_begin, unsigned column_nr)
      : _line_begin(line_begin), _column_begin(column_begin), _line_nr(line_nr),
        _column_nr(column_nr) {}
  ...
  get_input_location(const I &input,
...
constexpr auto get_input_location(
  ...
  while (true) {
    if (reader.position() == position) {
      // We've already found the position; it's at the beginning of a colum nor
      // newline. No need to do the expensive checks.
      //
      // This also allows `lexy_ext::shell` to work properly, if position is at
      // EOF, the reader.peek() call will ask for more input.
      break;
      ...
      if (lexy::_detail::min_range_end(column_begin.position(),
                                       newline_end.position(),
                                       position) != newline_end.position())
        break;
...
template <typename Counting, typename Input>
constexpr auto
get_input_location(const Input &input,
                   typename lexy::input_reader<Input>::iterator position) {
  return get_input_location<Counting>(input, position,
...
get_input_location(const Input &input,
                   typename lexy::input_reader<Input>::iterator position) {
  return get_input_location<_default_location_counting<Input>>(
      input, position, input_location_anchor(input));
}
} // namespace lexy
...
template <typename Counting, typename Input>
constexpr auto
get_input_line(const Input &input,
               typename lexy::input_reader<Input>::marker line_begin) {
  auto reader = input.reader();
  reader.reset(line_begin);

  auto line_end = reader.position();
  for (Counting counting; reader.peek() != decltype(reader)::encoding::eof() &&
                          !counting.try_match_newline(reader);
...
namespace lexy {
template <typename Input> struct input_line_annotation {
  ...
  /// true if the the range was spanning multiple line and needed to be
  /// truncated.
...
template <typename Input>
constexpr void
_get_input_line_annotation(input_line_annotation<Input> &result,
                           lexy::lexeme_for<Input> line,
                           lexy::lexeme_for<Input> newline,
...
constexpr auto
get_input_line_annotation(const Input &input,
                          const input_location<Input, Counting> &begin_location,
                          typename lexy::input_reader<Input>::iterator end)
    -> input_line_annotation<Input> {
  input_line_annotation<Input> result{};

  auto [line, newline] = _detail::get_input_line<Counting>(
      input, begin_location.anchor()._line_begin);
...
/// Computes the annotation for e.g. an error message covering [location,
/// location + size).
template <typename Input, typename Counting>
constexpr auto
get_input_line_annotation(const Input &input,
  ...
  input_line_annotation<Input> result{};
  auto [line, newline] =
      _detail::get_input_line<Counting>(input, location.anchor()._line_begin);
  ...
  auto end =
      _detail::next_clamped(location.position(), range_size, newline.end());
