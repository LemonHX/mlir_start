...
namespace lexy_ext {
template <typename TokenKind = void> class parse_tree_desc {
public:
  parse_tree_desc() : _level(0) { _tree += "\n"; }
  ...
  parse_tree_desc(RootProduction)
      : parse_tree_desc(lexy::production_name<RootProduction>()) {}

  template <typename Iterator>
  parse_tree_desc &token(lexy::token_kind<TokenKind> kind, Iterator begin,
                         Iterator end) {
    prefix();
  ...
  parse_tree_desc &token(lexy::token_kind<TokenKind> kind,
  ...
  template <typename CharT> parse_tree_desc &token(const CharT *spelling) {
    return token(lexy::unknown_token_kind, spelling);
  }
  template <typename CharT> parse_tree_desc &literal(const CharT *spelling) {
    return token(lexy::literal_token_kind, spelling);
  }
  template <typename CharT> parse_tree_desc &digits(const CharT *spelling) {
    return token(lexy::digits_token_kind, spelling);
  }
  template <typename CharT> parse_tree_desc &whitespace(const CharT *spelling) {
    return token(lexy::whitespace_token_kind, spelling);
  }

  parse_tree_desc &eof() { return token(lexy::eof_token_kind, ""); }
  ...
  static String convert(const parse_tree &tree) {
    ...
    for (auto [event, node] : tree.traverse())
      ...
      case lexy::traverse_event::leaf: {
        ...
        builder.token(token.kind(), token.lexeme().begin(),
                      token.lexeme().end());
        break;
      }
      }
