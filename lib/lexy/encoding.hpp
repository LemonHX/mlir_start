...
/// An encoding where the input is some 8bit encoding (ASCII, UTF-8, extended
/// ASCII etc.).
struct default_encoding {
  ...
  static LEXY_CONSTEVAL int_type eof() { return -1; }
...
/// An encoding where the input is assumed to be valid UTF-8, but the char
/// type is char.
...
struct utf16_encoding {
  ...
  static constexpr bool is_secondary_char_type() {
    return sizeof(wchar_t) == sizeof(char16_t) &&
           std::is_same_v<OtherCharType, wchar_t>;
  }
...
struct utf32_encoding {
  ...
  template <typename OtherCharType>
  static constexpr bool is_secondary_char_type() {
    return sizeof(wchar_t) == sizeof(char32_t) &&
           std::is_same_v<OtherCharType, wchar_t>;
  }

  static LEXY_CONSTEVAL int_type eof() {
    // The highest unicode code point is U+10'FFFF, so this is never a valid
    // code point.
...
struct byte_encoding {
  ...
  static constexpr bool is_secondary_char_type() {
    return std::is_same_v<OtherCharType, char> ||
           std::is_same_v<OtherCharType, std::byte>;
  }
...
//=== deduce_encoding ===//
namespace lexy {
template <typename CharT> struct _deduce_encoding;
template <typename CharT>
using deduce_encoding = typename _deduce_encoding<CharT>::type;

template <> struct _deduce_encoding<char> {
...
#if LEXY_HAS_CHAR8_T
template <> struct _deduce_encoding<LEXY_CHAR8_T> {
  using type = utf8_encoding;
};
#endif
template <> struct _deduce_encoding<char16_t> {
  using type = utf16_encoding;
};
template <> struct _deduce_encoding<char32_t> {
  using type = utf32_encoding;
};

template <> struct _deduce_encoding<unsigned char> {
  using type = byte_encoding;
};
template <> struct _deduce_encoding<std::byte> {
  using type = byte_encoding;
};
} // namespace lexy
...
constexpr auto is_unicode_encoding =
    std::is_same_v<Encoding, ascii_encoding> ||
    std::is_same_v<Encoding, utf8_encoding> ||
    std::is_same_v<Encoding, utf8_char_encoding> ||
    std::is_same_v<Encoding, utf16_encoding> ||
...
constexpr auto is_char_encoding =
    is_text_encoding<Encoding> || is_byte_encoding<Encoding>;

template <typename Encoding> constexpr auto is_node_encoding = false;
} // namespace lexy
...
constexpr bool is_compatible_char_type =
    std::is_same_v<typename Encoding::char_type, CharT> ||
...
template <typename CharT> constexpr bool is_ascii(CharT c) {
...
LEXY_CONSTEVAL TargetCharT transcode_char(CharT c) {
  if constexpr (std::is_same_v<CharT, TargetCharT>) {
    return c;
                     ...
                     std::is_same_v<TargetCharT, LEXY_CHAR8_T>) {
    // If we don't have char8_t, `LEXY_LIT(u8"ä")` would have the type char, not
    // LEXY_CHAR8_T (which is unsigned char). So we disable checking in that
    // case, to allow such usage. Note that this prevents catching
    // `LEXY_LIT("ä")`, but there is nothing we can do.
    ...
    // Note that we don't need to worry about signed/unsigned conversion, all
    // ASCII values are positive.
