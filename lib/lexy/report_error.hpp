...
template <typename Input> class diagnostic_writer {
public:
  explicit diagnostic_writer(const Input &input,
  ...
  OutputIt write_message(OutputIt out, diagnostic_kind kind,
  ...
  /// 100 | void foo();
  ///     |      ^^^ annotation
  template <typename OutputIt, typename Location, typename IteratorOrSize,
            typename Writer>
  OutputIt write_annotation(OutputIt out, annotation_kind kind,
                            const Location &begin_location, IteratorOrSize end,
                            const Writer &message) const {
    using namespace lexy::_detail;
    using lexy::_detail::color; // clang-cl bug
    ...
    *out++ = ' ';
    ...
    out = lexy::visualize_to(out, line.annotated,
                             _opts.reset(lexy::visualize_use_color));
    out = write_color<color::reset>(out, _opts);
    ...
    *out++ = ' ';
    ...
    // Then underline.
    auto underline_count =
        lexy::visualization_display_width(line.annotated, _opts);
...
OutputIt write_error(OutputIt out, const lexy::error_context<Input> &context,
  ...
  out = writer.write_message(
      out, diagnostic_kind::error,
  ...
  if (location.line_nr() != context_location.line_nr()) {
    out = writer.write_annotation(
        out, annotation_kind::secondary, context_location,
  ...
  if constexpr (std::is_same_v<Tag, lexy::expected_literal>) {
    auto string = lexy::_detail::make_literal_lexeme<typename Reader::encoding>(
        error.string(), error.length());

    out = writer.write_annotation(
        out, annotation_kind::primary, location, error.index() + 1,
    ...
    out = writer.write_annotation(
        out, annotation_kind::primary, location, error.end(),
        [&](OutputIt out, lexy::visualization_options opts) {
    ...
    out = writer.write_annotation(
        out, annotation_kind::primary, location, 1u,
        [&](OutputIt out, lexy::visualization_options) {
          out = lexy::_detail::write_str(out, "expected ");
...
namespace lexy_ext {
template <typename OutputIterator> struct _report_error {
  ...
  constexpr auto sink() const { return _sink{_iter, _opts, _path, 0}; }
  ...
  template <typename OI> constexpr _report_error<OI> to(OI out) const {
