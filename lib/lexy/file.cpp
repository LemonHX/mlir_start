...
#include "lib/lexy/input/file.hpp"
#include <cstddef>
...
public:
  ...
  operator int() const noexcept { return _file; }
...
lexy::file_error lexy::_detail::read_file(const char *path, file_callback cb,
                                          void *user_data) {
  ...
  if (size <= small_file_size) {
    ...
    if (::read(fd, builder.data(), builder.size()) !=
...
public:
  ...
  operator std::FILE *() const noexcept { return _file; }
...
lexy::file_error lexy::_detail::read_file(const char *path, file_callback cb,
  ...
  if (std::fread(builder.data(), sizeof(char), builder.size(), file) !=
      builder.size())
    return lexy::file_error::os_error;
  LEXY_ASSERT(std::fgetc(file) == EOF, "we haven't read everything?!");

  // Pass to callback.
  // Note that this isn't ideal, as we're having one unnecessary copy plus
  // allocation.
  cb(user_data, builder.data(), builder.size());
...
lexy::file_error lexy::_detail::read_stdin(file_callback cb, void *user_data) {
  ...
  while (true) {
    ...
    const auto read =
        std::fread(builder.write_data(), sizeof(char), buffer_size, stdin);
    builder.commit(read);
    ...
    // This grow might be unnecessary if we're just so happen to reach EOF with
    // the next input, but checking this requires reading more input.
    builder.grow();
  }
