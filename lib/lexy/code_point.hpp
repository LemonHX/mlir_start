...
public:
  ...
  //=== classification ===//
  constexpr bool is_ascii() const noexcept { return _value <= 0x7F; }
  constexpr bool is_bmp() const noexcept { return _value <= 0xFFFF; }
  constexpr bool is_valid() const noexcept { return _value <= 0x10'FFFF; }
...
#undef LEXY_UNICODE_CATEGORY
  ...
  template <general_category_t... Cats> struct _gc_group {
    const char *name;
...
#define LEXY_UNICODE_CATEGORY_GROUP(Name, Short, Long, ...)                    \
  ...
  LEXY_UNICODE_CATEGORY_GROUP("punctuation", P, punctuation, Pc, Pd, Ps, Pe, Pi,
                              Pf, Po);
...
general_category_name(lexy::code_point::general_category_t category) {
  switch (category) {
...
#if LEXY_HAS_UNICODE_DATABASE
#include "lib/lexy/_detail/unicode_database.hpp"

constexpr lexy::code_point::general_category_t
lexy::code_point::general_category() const noexcept {
  if (!is_valid())
