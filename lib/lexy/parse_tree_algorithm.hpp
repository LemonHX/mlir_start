...
namespace lexy_ext {
/// Returns a range that contains only the token nodes that are descendants of
/// the node. If the node is itself a token, returns a range that contains
...
auto tokens(
  ...
  public:
    ...
    class iterator
        : public lexy::_detail::forward_iterator_base<iterator, node_t, node_t,
                                                      void> {
    public:
      ...
      auto deref() const noexcept {
        ...
        LEXY_ASSERT(_cur->event == lexy::traverse_event::leaf,
                    "invalid increment");
        return _cur->node;
      }
      ...
      bool is_end() const noexcept { return _cur == _end; }
    ...
    explicit token_range(const traverse_range &range) noexcept
        : _begin(range) {}
...
auto find_covering_node(
  ...
  // This can be done more efficiently by pruning subtrees, but this should be
  // fast enough.
...
public:
  ...
  class iterator : public lexy::_detail::forward_iterator_base<iterator, node_t,
                                                               node_t, void> {
  public:
    iterator() noexcept = default;
  ...
  explicit _filtered_node_range(Predicate &&pred, Iterator begin,
...
template <typename Reader, typename TokenKind, typename MemoryResource,
          typename Predicate>
auto children(
    const lexy::parse_tree<Reader, TokenKind, MemoryResource> &,
    typename lexy::parse_tree<Reader, TokenKind, MemoryResource>::node node,
    Predicate predicate) {
  if constexpr (std::is_constructible_v<lexy::token_kind<TokenKind>, Predicate>)
    return _filtered_node_range(
        [kind = predicate](auto n) { return n.kind() == kind; },
...
/// For a production node, this is the position designated by a `dsl::position`
/// rule, or otherwise the position of its first non-empty child. If a
/// production node is empty, it returns a default constructed iterator.
template <typename Reader, typename TokenKind, typename MemoryResource>
auto node_position(
    const lexy::parse_tree<Reader, TokenKind, MemoryResource> &tree,
    typename lexy::parse_tree<Reader, TokenKind, MemoryResource>::node node) ->
