...
#include "lib/lexy/action/parse.hpp" // lexy::parse
#include "lib/lexy/callback.hpp"     // value callbacks
#include "lib/lexy/dsl.hpp"          // lexy::dsl::*
#include "lib/lexy/input/file.hpp"   // lexy::read_file

#include "lib/lexy/report_error.hpp" // lexy_ext::report_error
                                     //
namespace {
...
struct json_value {
  std::variant<json_null, json_bool, json_number, json_string, json_array,
               json_object>
      v;

  template <typename T> json_value(T t) : v(std::move(t)) {}
  ...
  void _print(json_null, int) const { std::fputs("null", stdout); }
...
struct number : lexy::token_production {
  ...
  static constexpr auto
      rule = dsl::peek(dsl::lit_c<'-'> / dsl::digit<>) >>
             dsl::p<integer> +
                 dsl::opt(dsl::p<fraction>) + dsl::opt(dsl::p<exponent>);
...
struct string : lexy::token_production {
  ...
  static constexpr auto value =
      lexy::as_string<ast::json_string, lexy::utf8_encoding>;
};
...
struct array {
  // A (potentially empty) list of json values, seperated by comma and
  // surrounded by square brackets.
  static constexpr auto rule = dsl::square_bracketed.opt_list(
      dsl::recurse<json_value>,
...
struct object {
  static constexpr auto rule = [] {
    // We try parsing the colon. This means that a missing colon raises an
    // error, which is then caught and parsing continues as if nothing happens.
    // Without the try, parsing the current item would be canceled immediately.
    auto item =
        dsl::p<string> + dsl::try_(dsl::colon) + dsl::recurse<json_value>;

    // Trailing seperators are not allowed.
    // Use `dsl::trailing_sep()` if you want to allow it.
    auto sep = dsl::sep(dsl::comma).trailing_error<unexpected_trailing_comma>;

    // A (potentially empty) list of items, seperated by comma and surrounded by
    // curly brackets.
...
struct json_value : lexy::transparent_production {
  ...
  struct expected_json_value {
    static LEXY_CONSTEVAL auto name() { return "expected json value"; }
  };

  static constexpr auto rule = [] {
    auto primitive =
        dsl::p<null> | dsl::p<boolean> | dsl::p<number> | dsl::p<string>;
...
#include "lib/lexy/input/string_input.hpp" // lexy::string_input

int main() {

  // We're requiring UTF-8 input.
  auto input = lexy::zstring_input<lexy::utf8_encoding, char>("{\"hi\": 42}");
  auto json = lexy::parse<grammar::json>(
      input, lexy_ext::report_error.path("./error.txt"));
  if (json.has_value())
    json.value().print();
